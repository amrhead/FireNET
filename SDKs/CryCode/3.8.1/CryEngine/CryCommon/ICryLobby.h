...
#include <ICryTCPService.h> // <> required for Interfuscator
#include <ISystem.h>
...
public:
	CLobbyString() : string() {}
	CLobbyString(uint32 n) : string() {char buff[32]; sprintf_s(buff, "%u", n); assign(buff); }
	CLobbyString(string s) : string(s) {}
	CLobbyString(uint16 n) : string() {char buff[32]; sprintf_s(buff, "%u", n); assign(buff); }
	CLobbyString(int n) : string() {char buff[32]; sprintf_s(buff, "%i", n); assign(buff); }
...
// A lobby service may query the game for the usage of each datum registered
// with SessionRegisterUserData.
...
// SCryLobbySessionAdvertisement
// Used for creating a session and updating the session details.
// Note: in SDK 1.500 the linkdata (m_pData) could not be updated once created. Feature is due to be added in 1.700.
// Note2: Updating localised session name/status will overwrite/replace all previous name/status values. Values not updated are discarded.
// Note3: Is to NOT possible to update the number of slots, private or editable flags after creation.
...
// SCryLobbyAgeRestrictionSetup
// A request to fill supply age restriction settings for different regions.
// m_numCountries - number of specified countries.
// m_pCountries - array of m_numCountries SCryLobbyAgeRestrictionSetup::SCountry entries. 
struct SCryLobbyAgeRestrictionSetup
{
	...
	int32							m_numCountries;
	const SCountry*		m_pCountries;
};
