...
template<typename F> struct Matrix33_tpl
{
	...
	explicit ILINE Matrix33_tpl<F>(const Quat_tpl<F>& q) 
	{
		...
		F xx=1-v2.x*q.v.x;	F yy=v2.y*q.v.y;		F xw=v2.x*q.w;
		F xy=v2.y*q.v.x;		F yz=v2.z*q.v.y;		F yw=v2.y*q.w;
		F xz=v2.z*q.v.x;		F zz=v2.z*q.v.z;		F zw=v2.z*q.w;
	...
	template<class F1> explicit ILINE Matrix33_tpl<F>( const Quat_tpl<F1>& q) 
	{
		...
		F1 xx=1-v2.x*q.v.x;		F1 yy=v2.y*q.v.y;			F1 xw=v2.x*q.w;
		F1 xy=v2.y*q.v.x;			F1 yz=v2.z*q.v.y;			F1 yw=v2.y*q.w;
		F1 xz=v2.z*q.v.x;			F1 zz=v2.z*q.v.z;			F1 zw=v2.z*q.w;
	...
	*/
	...
	{
		Matrix33_tpl<F> m33;	m33.SetRotationAA(axis); return m33;	
	}
	...
	*/
	ILINE void SetRotationV0V1( const Vec3_tpl<F>& v0, const Vec3_tpl<F>& v1 ) 
	{
		...
		{
			Vec3_tpl<F> v = v0%v1;	F h = 1/(1 + dot); 
	...
	{	
		Matrix33_tpl<F> m33;	m33.SetRotationV0V1(v0,v1); return m33;	
	}
	...
	*/
	ILINE void SetSlerp( const Matrix33_tpl<F>& m, const Matrix33_tpl<F>& n, F t)
	{
		...
		F l = sqrt(axis|axis);	if (l>F(0.00001) ) axis/=l;	else axis(1,0,0); 
	...
	{	
		Matrix33_tpl<F> m33;	m33.SetSlerp(m,n,t); return m33;	
	}
	...
	*/
	...
	{	
		Matrix33_tpl<F> dst=*this; dst.Invert(); return dst;	
	}
