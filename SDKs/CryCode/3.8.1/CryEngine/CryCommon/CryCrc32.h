// Copyright 2004-2015 Crytek GmbH. All rights reserved.
...
public:
	...
	void Add(const void* const pData, size_t sizeInBytes)
	{
...
#define CRY_TMP_ASCII_TO_LOWER(c)  uint8(((c) <= 'Z' && (c) >= 'A') ? (c) + ('a' - 'A') : (c))
	...
	void AddLowercase(const char* pData, size_t sizeInBytes)
	{
		while (sizeInBytes--)
		{
			const uint8 c = *pData++;
	...
	void AddLowercase(const char* szData)
	{
		while (*szData)
		{
