...
template <typename F> struct Vec3_tpl
{
  ...
  ILINE Vec3_tpl( const Vec3_tpl& v )	{	x=v.x; y=v.y; z=v.z; }
	template<class F1> ILINE  Vec3_tpl<F>( const Vec3_tpl<F1>& v ) 	{ 	x=F(v.x); y=F(v.y); z=F(v.z); 	assert(IsValid()); 	}

	ILINE Vec3_tpl<F>(const Vec2_tpl<F>& v) { 	x=v.x; y=v.y; z=0;	assert(IsValid()); 	}
	template<class T> ILINE Vec3_tpl<F>(const Vec2_tpl<T>& v) {	x=F(v.x); y=F(v.y); z=0;	assert(IsValid()); }

	explicit ILINE Vec3_tpl<F>(const Ang3_tpl<F>& v) {  x=v.x; y=v.y; z=v.z; assert(IsValid());	}
	template<class T> explicit ILINE Vec3_tpl<F>(const Ang3_tpl<T>& v) {  x=v.x; y=v.y; z=v.z; assert(IsValid());	}
