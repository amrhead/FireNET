...
inline bool ScriptAnyValue::operator==( const ScriptAnyValue& rhs )const
{
	...
	if (result)
	{
		switch (type)
		{
		case ANY_TBOOLEAN: result = b == rhs.b; break;
		case ANY_TNUMBER: result = number == rhs.number; break;
		case ANY_TSTRING: result = str == rhs.str; break;
		case ANY_TVECTOR: result = vec3.x == rhs.vec3.x && vec3.y == rhs.vec3.y && vec3.z == rhs.vec3.z; break;
		case ANY_TTABLE: result = table == rhs.table; break;
		case ANY_THANDLE: result = ptr == rhs.ptr; break;
		case ANY_TFUNCTION: result = gEnv->pScriptSystem->CompareFuncRef(function, rhs.function); break;
		case ANY_TUSERDATA: result = ud.nRef == rhs.ud.nRef && ud.ptr == rhs.ud.ptr; break;
		}
	}
	return result;
}
