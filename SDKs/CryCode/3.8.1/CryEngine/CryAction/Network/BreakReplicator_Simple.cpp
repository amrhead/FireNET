...
void PlaneBreak::SerializeWith( CBitArray& array )
{
	...
	m_be.partid[1] = (m_be.partid[1]/PARTID_CGA) << 8 | (m_be.partid[1] % PARTID_CGA);
	...
	m_be.partid[1] = (m_be.partid[1]&0xff) + (m_be.partid[1] >> 8)*PARTID_CGA;
...
void PlaneBreak::Playback()
{
	...
	epc.pt=m_be.pt; epc.n=m_be.n;
	epc.vloc[0]=m_be.vloc[0]; epc.vloc[1]=m_be.vloc[1];
	epc.mass[0]=m_be.mass[0]; epc.mass[1]=m_be.mass[1];
	epc.idmat[0]=m_be.idmat[0]; epc.idmat[1]=m_be.idmat[1];
	epc.partid[0]=m_be.partid[0]; epc.partid[1]=m_be.partid[1];
...
CBreakReplicator::CBreakReplicator(CGameContext* pGameCtx)
{
	...
	s_dummyStream.m_pent = (IPhysicalEntity*)0xCBADBEEF;
...
void CBreakReplicator::OnEndFrame()
{
	...
	int i = m_activeStartIdx;
	const int lastBreak = m_streams.size();
	while (i<lastBreak && m_streams[i]->m_mode==BreakStream::k_finished) i++;
	m_activeStartIdx=i;
