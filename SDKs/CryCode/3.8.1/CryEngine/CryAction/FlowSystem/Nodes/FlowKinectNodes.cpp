...
class CFlowNode_KinectAlignment : public IKinectInputListener, public CFlowBaseNode<eNCT_Instanced>
{
public:
...
class CFlowNode_KinectSkeleton : public CFlowBaseNode<eNCT_Instanced>
{
	public:

		CFlowNode_KinectSkeleton(SActivationInfo *pActivationInfo) : m_frequency(0.0f)
		{
		}
...
namespace stl
{
	template <class COMPARE> struct hash_compare<string, COMPARE>
	{
	...
	private:
		...
		class CParam
		{
			public:
				...
				inline CParam() : m_type(ePT_Nil)
				{
				}

				inline CParam(int32 rhs) : m_type(ePT_Int32), m_int32(rhs)
				{
				}

				inline CParam(float rhs) : m_type(ePT_Float), m_float(rhs)
				{
				}

				inline CParam(const ICVar *rhs) : m_type(ePT_CVar), m_pCVar(rhs)
				{
				}
...
class CKinectGestureSystem
{
	...
	private:
		...
		typedef stl::hash_map<string, CKinectPose> TPoseMap;

		typedef stl::hash_map<string, CKinectGesture> TGestureMap;
