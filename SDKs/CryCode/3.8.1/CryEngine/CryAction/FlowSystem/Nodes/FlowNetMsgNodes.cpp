		...
		case eFE_Activate:
			if (IsPortActive(pActInfo,IN_SEND))
			{
				...
				if (target != SNetMsgData::eT_Invalid)
				{
					...
					{
						CryLog("[NetMsg] CFlowNode_MsgSender: %s Send message [%s/%s]", CNetMessageDistpatcher::DbgTranslateLocationToString(), key.c_str(), value.c_str());
...
public:
	...
	virtual void OnMessage(const SNetMsgData & data) override
	{
		...
		if (data.key == key)
		{
			CryLog("[NetMsg] CFlowNode_MsgReceiver: %s Listener received message [%s->%s]-[%s/%s]", CNetMessageDistpatcher::DbgTranslateLocationToString(), data.DbgTranslateSourceToString(),  data.DbgTranslateTargetToString(), key.c_str(), data.value.c_str());
