...
//CryWatch3DAdd(GetHitType(hitInfo.type), hitInfo.pos, 2.f, NULL, -1.f);
	...
	if (!hitInfo.remote && hitInfo.targetId && !backface && !dead)
	{
		...
		hitToSend.time = m_cachedServerTime.GetSeconds();
#endif
...
#if defined(ANTI_CHEAT)
				hitToSend.shotId = pProjectile->GetShotId();
#endif
			}
...
void CGameRules::ClientExplosion(SExplosionContainer &explosionContainer)
{
	...
	if (gEnv->IsClient())
	{
		...
		if (pClientActor)
		{
			...
			// Inform the music logic of an explosion event.
			if (gEnv->pMusicSystem)
			{
				if (gEnv->pMusicSystem->GetMusicLogic())
				{
					gEnv->pMusicSystem->GetMusicLogic()->SetEvent(eMUSICLOGICEVENT_EXPLOSION, distSq);
				}
			}
		}
	}
		...
		if (pLobby)
		{
			...
			if (pMatchMaking)
			{
				...
				if (pGameLobby)
				{
					...
					{
						CryLog("CGameRules::UpdateNetLimbo() GameState:%d timing out server connection, actual timeSinceReceived=%lld", (int)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
					...
					else if (packetFromServerTime > 5.0f)
					{
						...
						{
							CryLog("CGameRules::UpdateNetLimbo() GameState:%d 5.0f seconds timing out server connection, actual timeSinceReceived=%lld", (int)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
						}
					}
					else if (packetFromServerTime > 3.0f)
					{
						...
						{
							CryLog("CGameRules::UpdateNetLimbo() GameState:%d 3.0f seconds timing out server connection, actual timeSinceReceived=%lld", (int)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
						}
					}
					else if (packetFromServerTime > 2.0f)
					{
						...
						{
							CryLog("CGameRules::UpdateNetLimbo() GameState:%d 2.0f seconds timing out server connection, actual timeSinceReceived=%lld", (int)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
						}
					}
...
IMPLEMENT_RMI(CGameRules, SvRequestHit)
{
	...
	bool bCallServerHit = true;

	if(CServerCheatMonitor * pServerCheatMonitor = g_pGame->GetServerSideCheatMonitor())
	{
		bCallServerHit = pServerCheatMonitor->EvaluateHitValidity(pNetChannel, info);
	}
