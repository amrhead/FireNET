...
#if 0
  // Very basic unit test code for CryFixedCircularBuffer
	typedef CryFixedCircularBuffer<int,10> intCircularBuffer;
	intCircularBuffer intBuffer;
	...
	int idx=0;
	intCircularBuffer::iterator intBufStart(intBuffer.begin());
	intCircularBuffer::iterator intBufEnd(intBuffer.end());
	for(intCircularBuffer::iterator itr = intBufStart; itr!=intBufEnd; itr++)
	{
		int val = *itr;
		CryLogAlways("\nIteration:%d val:%d", idx, val);
		idx++;
	}

	for(int i=0;i<9;i++)
	{
		intCircularBuffer::iterator newFrontIterator = intBuffer.pop_front();
		CryLogAlways("\nPopped Front: frontVal-%d backVal-%d size:%d", *newFrontIterator, intBuffer.back(), intBuffer.size());
	}
#endif
}
