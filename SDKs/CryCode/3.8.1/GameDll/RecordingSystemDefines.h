...
{
	SKillCamExpectedData ( ) { Reset(); }
	SKillCamExpectedData ( EntityId sender, EntityId victim, bool winningKill ) : m_sender(sender), m_victim(victim), m_winningKill(winningKill) {}
	ILINE void Reset ( ) { m_sender = m_victim = 0; m_winningKill = false; }
	ILINE const bool operator == (const SKillCamExpectedData& o) const { return m_sender==o.m_sender && m_victim==o.m_victim && m_winningKill==o.m_winningKill; }
