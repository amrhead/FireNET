...
void CGameRulesStandardRounds::Init( XmlNodeRef xml )
{
	...
	if (xml->getAttr("playMusicAtEndOfRound", iScratch))
	{
		m_bPlayMusicAtEndOfRound = (iScratch != 0);
	}
...
void CGameRulesStandardRounds::ClDisplayEndOfRoundMessage()
{
	...
	if (!bShowRoundsAsDraw && m_previousRoundWinnerTeamId)
	{
		...
		if (localTeam == m_previousRoundWinnerTeamId)
		{
			...
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_RoundWon");
		...
		else
		{
			...
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_RoundLost");
	...
	else
	{
		...
		musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_RoundDrawn");
	...
	if (!m_bPlayMusicAtEndOfRound)
	{
		musicSignal = INVALID_AUDIOSIGNAL_ID;
	}

	CAudioSignalPlayer::JustPlay("RoundEndOneShot"); 
	CAudioSignalPlayer::JustPlay("Multiplayer_Music_StopMusicImmediately"); // stop any music that's playing
	SHUDEventWrapper::OnRoundEnd(winner, clientScoreIsTop, m_victoryDescMessage.c_str(), m_victoryMessage.c_str(), musicSignal, announcement);
