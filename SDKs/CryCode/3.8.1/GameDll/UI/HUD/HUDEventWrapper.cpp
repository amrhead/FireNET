...
void SHUDEventWrapper::OnRoundEnd( const int winner, const bool clientScoreIsTop, const char* victoryDescMessage, const char* victoryMessage, const TAudioSignalID roundEndMusicSignal, const EAnnouncementID announcement )
{
	if (!gEnv->IsDedicated())
	{
...
void SHUDEventWrapper::OnGameEnd(const int teamOrPlayerId, const bool clientScoreIsTop, EGameOverReason reason, const char* message, ESVC_DrawResolution drawResolution, const TAudioSignalID musicSignal, const EAnnouncementID announcement)
{
	if (!gEnv->IsDedicated())
	{
