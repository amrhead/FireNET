...
bool SPlaylistRotationExtension::LoadFromXmlNode(const XmlNodeRef xmlNode)
{
	...
	if (const XmlNodeRef infoNode=xmlNode->findChild("ExtendedInfo"))
	{
		...
		{
			LOCAL_WARNING(0, string().Format("Level Rotation is a Playlist but its \"PlaylistInfo\" entry doesn't have an \"enabled\" attribute"));
		}
		...
		LOCAL_WARNING(uniqueName, string().Format("Level Rotation is a Playlist but its \"PlaylistInfo\" entry doesn't have a \"uniqueName\" attribute"));
	...
	{
		LOCAL_WARNING(0, string().Format("Level Rotation xmlNode doesn't have a \"ExtendedInfo\" entry"));
	}
...
void CPlaylistManager::AddPlaylistsFromPath(const char* pPath)
{
			...
			if (XmlNodeRef child=indexRoot->getChild(i))
			{
				...
				if (stricmp(pTag, "PlaylistFile") == 0)
				{
					...
					if (const char* pName = allow ? child->getAttr("name") : NULL)
					{
						...
						{
							LOCAL_WARNING(0, string().Format("Failed to load '%s'!", filePath.c_str()));
						}
					}
					else
					{
						LOCAL_WARNING(!allow, string().Format("PlaylistFile element '%s' in '%s' didn't have a \"name\" attribute", pTag, indexPath.c_str()));
					}
				}
				else
				{
					LOCAL_WARNING(0, string().Format("Unexpected xml child '%s' in '%s'", pTag, indexPath.c_str()));
				}
	...
	{
		LOCAL_WARNING(0, string().Format("Couldn't open playlists index file '%s'", indexPath.c_str()));
	}
...
void CPlaylistManager::AddPlaylistFromXmlNode(XmlNodeRef xmlNode)
{
	...
	if (p->rotExtInfo.LoadFromXmlNode(xmlNode))
	{
		...
		{
			LOCAL_WARNING(0, string().Format("Error adding loaded playlist as a playlist with id '%u' has already been added!", genId));
			m_playlists.pop_back();
		}
	}
	else
		...
		for (int i=0; i<n; i++)
		{
			if (XmlNodeRef child=indexRoot->getChild(i))
			{
				...
				if (stricmp(pTag, "VariantsFile") == 0)
				{
					...
					{
						LOCAL_WARNING(0, string().Format("VariantsFile element '%s' in '%s' didn't have a \"name\" attribute", pTag, indexPath.c_str()));
					}
				}
				else
				{
...
void CPlaylistManager::LoadVariantsFile(const char *pPath)
{
	...
	{
		LOCAL_WARNING(0, string().Format("Failed to load '%s'!", pPath));
	}
}
...
void CPlaylistManager::AddVariantFromXmlNode( XmlNodeRef xmlNode )
{
	if (!stricmp(xmlNode->getTag(), "Variant"))
	{
		...
		if (xmlNode->getAttr("name", &pName))
		{
			...
			for (int i = 0; i < numChildren; ++ i)
			{
				...
				if (!stricmp(xmlChild->getTag(), "Options"))
				{
					...
					for (int j = 0; j < numOptions; ++ j)
					{
						...
						if (!stricmp(xmlOption->getTag(), "Option"))
						{
							...
							{
								LOCAL_WARNING(0, string().Format("AddVariantFromXmlNode: Expected 'setting' attribute, inside variant '%s'", newVariant.m_name.c_str()));
								bOk = false;
							}
						}
						else
						{
							LOCAL_WARNING(0, string().Format("AddVariantFromXmlNode: Expected 'Option' tag, got '%s' instead, inside variant '%s'", xmlOption->getTag(), newVariant.m_name.c_str()));
							bOk = false;
						}
					}
				}
					...
					for (int j = 0; j < numModes; ++ j)
					{
						...
						if (!stricmp(xmlMode->getTag(), "Mode"))
						{
							...
							if (xmlMode->getAttr("name", &pSetting))
							{
								...
								{
									LOCAL_WARNING(0, string().Format("AddVariantFromXmlNode: Unknown mode '%s' found, inside variant '%s'", pSetting, newVariant.m_name.c_str()));
									bOk = false;
								}
							}
						}
						...
						{
							LOCAL_WARNING(0, string().Format("AddVariantFromXmlNode: Expected 'Mode' tag, got '%s' instead, inside variant '%s'", xmlMode->getTag(), newVariant.m_name.c_str()));
							bOk = false;
						}
					}
				}
				...
				{
					LOCAL_WARNING(0, string().Format("AddVariantFromXmlNode: Unexpected '%s' tag, inside variant '%s'", xmlChild->getTag(), newVariant.m_name.c_str()));
					bOk = false;
				}
			}
			if (bOk)
	...
	{
		LOCAL_WARNING(0, string().Format("AddVariantFromXmlNode: Expected node tag to be 'Variant', got %s instead", xmlNode->getTag()));
	}
}
	...
	for (int i = 0; i < numVars; ++ i)
	{
		if (m_configVars[i].m_bNetSynched)
		{
			...
			case CVAR_STRING:
				...
				const uint8 length = strlen(m_configVars[i].m_pCVar->GetString()) + 1; //+1 == escape character
...
void CPlaylistManager::GetLocalisationSpecificAttr(const XmlNodeRef& rNode, const char* pAttrKey, const char* pLang, T* pOutVal)
{
	...
	if (rNode)
	{
		if (!useEnglish)
		{
			...
			{
				useEnglish = true;
				LOCAL_WARNING(0, string().Format("Could not get attribute '%s' for language '%s', will try falling-back to English...", pAttrKey, pLang));
			}
		}
		...
		if (useEnglish)
		{
			...
			LOCAL_WARNING(gotEng, string().Format("Could not get attribute '%s' for English language! Will not have a valid value for this attribute.", pAttrKey));
		}
	}
	...
	{
		LOCAL_WARNING(0, string().Format("Could not get an attribute named '%s' in node '%s' (when pAttrKey='%s' and pLang='%s')", attrKeyWithLang.c_str(), (rNode ? rNode->getTag() : NULL), pAttrKey, pLang));
		if (pOutVal)
		{
			(*pOutVal).clear();
		}
...
void CPlaylistManager::LoadLevelRotation()
{
	...
	levelRotationFileName.Format("./%sLevelRotation.xml", path.c_str());
