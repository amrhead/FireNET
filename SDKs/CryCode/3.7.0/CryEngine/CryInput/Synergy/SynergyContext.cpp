...
static bool synergyConnectFunc(CSynergyContext* pContext)
{
	...
	if (pContext->m_socket>=0)
	{
		...
		{
			int result = CrySock::connect(pContext->m_socket, (const CRYSOCKADDR*)&sa, sizeof(sa));
		...
		for (i=0; i<sizeof(s_packets)/sizeof(s_packets[0]); i++)
		{
			...
			if (packetLen>=len && memcmp(s->GetData(), s_packets[i].pattern, len)==0)
			{
				...
				while (!bDone)
				{
					...
					{
					case ARG_UINT8: args[numArgs++]=s->ReadU8(); break;
					case ARG_UINT16: args[numArgs++]=s->ReadU16(); break;
					case ARG_UINT32: args[numArgs++]=s->ReadU32(); break;
					case ARG_END: bDone=true; break;
					}
				}
