...
static void IterTable(lua_State *L, IScriptTableDumpSink *p)
{
	...
	while(lua_next(L, -2))
	{
		...
		if(keyType == LUA_TSTRING)
		{
			...
			switch(valType)
			{
			case LUA_TNIL: p->OnElementFound(key, svtNull); break;
			case LUA_TBOOLEAN: p->OnElementFound(key, svtBool); break;
			case LUA_TLIGHTUSERDATA: p->OnElementFound(key, svtPointer); break;
			case LUA_TNUMBER: p->OnElementFound(key, svtNumber); break;
			case LUA_TSTRING: p->OnElementFound(key, svtString); break;
			case LUA_TTABLE:
				if(strcmp(key, "__index") != 0)
					p->OnElementFound(key, svtObject);
				break;
			case LUA_TFUNCTION: p->OnElementFound(key, svtFunction); break;
			case LUA_TUSERDATA: p->OnElementFound(key, svtUserData); break;
			}
		}
		else
		{
			...
			switch(valType)
			{
			case LUA_TNIL: p->OnElementFound(idx, svtNull); break;
			case LUA_TBOOLEAN: p->OnElementFound(idx, svtBool); break;
			case LUA_TLIGHTUSERDATA: p->OnElementFound(idx, svtPointer); break;
			case LUA_TNUMBER: p->OnElementFound(idx, svtNumber); break;
			case LUA_TSTRING: p->OnElementFound(idx, svtString); break;
			case LUA_TTABLE: p->OnElementFound(idx, svtObject); break;
			case LUA_TFUNCTION: p->OnElementFound(idx, svtFunction); break;
