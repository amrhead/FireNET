...
namespace ATLUtils
{
	void SetSwitchState( const char* switchIdName, const char* switchValue, IEntityAudioProxyPtr pIEntityAudioProxy )
	{
		...
		TAudioControlID switchControlId( INVALID_AUDIO_CONTROL_ID );
		pAudioSystem->GetAudioSwitchID( switchIdName, switchControlId );
		if( switchControlId )
		{
			TAudioSwitchStateID switchStateId( INVALID_AUDIO_SWITCH_STATE_ID );
...
void CommunicationHandler::Reset()
{
	// Notify sound/voice listeners that the sounds have stopped
	{
		...
		for (PlayingSounds::iterator it(m_playingSounds.begin()); it != end; ++it)
		{
			...
			{
				pIEntityAudioProxy->ExecuteTrigger(playingSound.correspondingStopControlId, eLSM_None);
			}
		}
...
void CommunicationHandler::StopSound(const SCommunicationSound& soundToStop)
{
	...
	if( pEntity )
	{
		...
		if( pIEntityAudioProxy )
		{
			...
			{
				pIEntityAudioProxy->ExecuteTrigger(soundToStop.stopSoundControlId, eLSM_None);
			}
			else
			{
...
SCommunicationSound CommunicationHandler::PlaySound(CommPlayID playID, const char* name, ESoundType type, ELipSyncMethod lipSyncMethod, IEventListener* listener)
{
	...
	if( pEntity )
	{
		...
		if( pIEntityAudioProxy )
		{
			...
			assert(gEnv && gEnv->pAudioSystem);
			IAudioSystem* pAudioSystem = gEnv->pAudioSystem;
			TAudioControlID playCommunicationControlId( INVALID_AUDIO_CONTROL_ID );
			TAudioControlID stopCommunicationControlId( INVALID_AUDIO_CONTROL_ID );
...
void CommunicationHandler::OnSoundTriggerFinishedToPlay(const TAudioControlID nTriggerID)
{
