...
namespace Distance {
  ...
  ILINE Vec3_tpl<F> Origin_Triangle2D( const Triangle_tpl<F>& t ) {
	...
	ILINE hwvec3 Origin_Triangle2D( const hwvec3& a, const hwvec3& b, const hwvec3& c ) {
		...
		case 1:
			{
				if(SIMDFLessThanB(HWV3AsSIMDF(vZero), HWV3Dot(a, vBSubA))) flag=5;
				else if (SIMDFLessThanB(HWV3AsSIMDF(vZero), HWV3Dot(b, vASubB))) flag=3;
				break;
			}
		case 2:	
			{
				if (SIMDFLessThanB(HWV3AsSIMDF(vZero), HWV3Dot(b, vCSubB))) flag=3;
				else if (SIMDFLessThanB(HWV3AsSIMDF(vZero), HWV3Dot(c, vBSubC))) flag=6;
				break;
			}
		case 3:	return b; //vertex B is closed
		case 4:	
			{
				if (SIMDFLessThanB(HWV3AsSIMDF(vZero), HWV3Dot(c, vASubC))) flag=6;
				else if (SIMDFLessThanB(HWV3AsSIMDF(vZero), HWV3Dot(a, vCSubA))) flag=5;
				break;
			}
		case 5:	return a; //vertex A is closed 
		case 6:	return c; //vertex C is closed
