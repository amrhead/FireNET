...
void CShark::ProcessEvent(SEntityEvent& event)
{
	...
	{
		CreateScriptEvent("hide", event.event == ENTITY_EVENT_HIDE ? 1.f : 0.f);
	}
...
//	IAnimationGraphState* pAGState = GetAnimationGraphState();
	...
	MeleeAnimation();
	m_animationSpeedAttack = fast ? 1.5f : 1.0f;
...
//		if (m_input.viewVector.len2()>0.0f)
	...
	*/
	...
	if(m_rollTime >0)
	{
		...
		m_roll = (1 - cosf((m_params.attackRollTime - m_rollTime)/m_params.attackRollTime * 2*gf_PI ) )/2 * m_rollMaxAngle;
		float straightFactor = fabs(m_turnAnimBlend - 0.5f);
...
//	float angVel = m_stats.angVelocity.z;
	...
	{
		turnAnimBlend = 0.5f + sgn(CrossZ(curRot,m_lastRot))*0.5f/(m_stats.turnRadius/m_params.minTurnRadius);
	}
	...
	else
	{
		if(maxSpeed==0)
			...
			animSpeed = CLAMP(m_stats.speed/maxSpeed,0.5f,1.f)*1.2f;
	}
...
void CShark::Revive(EReasonForRevive reasonForRevive)
{
	...
	/*
	if (m_params.trailEffect[0] && gEnv->pParticleManager->FindEffect(m_params.trailEffect))
	{
...
{
	CHANGED_NETWORK_STATE(this,  eEA_GameServerStatic | eEA_GameServerDynamic | eEA_GameClientStatic | eEA_GameClientDynamic );
...
void CShark::UpdateScriptStats(CScriptSetGetChain& stats)
{
	CActor::UpdateScriptStats(stats);
}
...
void CShark::SetParams(SmartScriptTable &rTable,bool resetFirst)
{
	...
	*/
/*	if (rTable->GetValue("turnSound",str))
	{ 
...
void CShark::FindEscapePoints()
{
	...
	if(m_params.escapeAnchorType && m_targetId)
	{
		...
		if(pTarget)
		{
			...
			AutoAIObjectIter it(pAIObjMgr->GetFirstAIObjectInRange(OBJFILTER_TYPE, m_params.escapeAnchorType, pTarget->GetWorldPos(), 400, true));
