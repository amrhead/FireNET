...
CGameRulesObjective_Predator::CGameRulesObjective_Predator()
	...
	, m_fMinimalReviveTime(0.f)
	, m_fSoldierSuicideRespawnTimeMult(0.f)
	, m_fLateJoiningGracePeriod(15.f)
	, m_fDelayedAnnouncementTimer(0.f)
...
void CGameRulesObjective_Predator::Client_OnSoldiersRemainingCountChanged(int previousCount)
{
	...
	if (pActor)
	{
		...
		if ((m_numRemainingSoldiers == 1) && (previousCount > 1))
		{
			...
			if (pVictoryConditions)
			{
				pVictoryConditions->SetCanPlayMusicMoods(false);
			}
		}
...
void CGameRulesObjective_Predator::OnRoundEnd()
{
	...
	if (pVictoryConditions)
	{
		pVictoryConditions->SetCanPlayMusicMoods(true);
	}
