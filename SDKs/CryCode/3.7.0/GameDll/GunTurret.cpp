...
void CGunTurret::OnTargetLocked(IEntity* pTarget)
{
  ...
  if (IsServer())
  {
	...
	if(IsClient())
	{
		gEnv->pMusicSystem->GetMusicLogic()->SetEvent(eMUSICLOGICEVENT_PLAYER_TURRET_ATTACK);
	}
}
...
bool CGunTurret::QueryDeployment(const Vec3 &pos, const Vec3 &direction, float radius, Vec3 &hitPos)
{
	...
	gEnv->pRenderer->GetIRenderAuxGeom()->DrawSphere(pos, radius, fromCol);
	...
	gEnv->pRenderer->GetIRenderAuxGeom()->DrawSphere(pos + (direction * d), radius, fail ? hitCol : clearCol);
	gEnv->pRenderer->GetIRenderAuxGeom()->DrawLine(pos, fromCol, pos + (direction * d), fail ? hitCol : clearCol);
	if (fail)
	{
		return false;		
	}
	...
	fail = (d == 0.0f);
	gEnv->pRenderer->GetIRenderAuxGeom()->DrawSphere(sweepSphere.center + (DOWN * d), radius, fail ? hitCol : clearCol);
	gEnv->pRenderer->GetIRenderAuxGeom()->DrawLine(sweepSphere.center, fromCol, sweepSphere.center + (DOWN * d), fail ? hitCol : clearCol);
	if (fail)
	{
		return false;
	}
