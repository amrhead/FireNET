...
namespace
{
	...
	{
		static const int PARTID_ATTACHMENTS = 1000;
		ICharacterInstance* pCharInst = entity.GetCharacter(0);
		int jointId = partId;
		if(partId<PARTID_ATTACHMENTS)
		{
		}
		else if(partId<PARTID_CGA)
		{
			jointId = partId - PARTID_ATTACHMENTS;
			CRY_ASSERT(pCharInst);
			if(pCharInst)
			{
				if(IAttachment* pAttachment = pCharInst->GetIAttachmentManager()->GetInterfaceByIndex(jointId))
				{
					if(pReturnPos)
					{
