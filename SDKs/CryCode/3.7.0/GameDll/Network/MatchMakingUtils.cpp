...
namespace MatchmakingUtils
{
	...
	void ShowAchievementsUI()
	{
		...
		if ( user != nullptr )
		{
			...
			HRESULT hr = user->GetPlatformUser( &pUser );

			hr = Interface::Statics<SystemUI>()->LaunchAchievementsAsync( pUser.Get(), 0x65D9B841, &pAction );
...
#define MAX_PARTY 3
	...
	void ShowPartyUI()
	{
		...
		if ( user != nullptr )
		{
			...
			HRESULT hr = user->GetPlatformUser( &pUser );
			hr = Interface::Statics<SystemUI>()->ShowSendInvitesAsync( pUser.Get(), &pAction );
	...
	concurrency::task< HRESULT > EnsureSDAAvailable( std::shared_ptr< Live::State::User > user )
	{
			...
			[]()
		{
			...
			if( std::cv_status::timeout == cv.wait_for( lk, std::chrono::seconds(60), [](){ return !!complete; } ) )
				complete = true;
	...
	wstring GetGamertag( const unsigned long long xuid )
	{
		...
		if (user != nullptr)
		{
			if (xuid == user->Xuid())
			{
				...
				ComPtr<IUser> pUser;
				HRESULT hr = Interface::Statics<IUser>()->GetUserById( userId, pUser.GetAddressOf() );

				ComPtr<IUserDisplayInfo> dispInfo;
				hr = pUser->get_DisplayInfo( dispInfo.GetAddressOf() );

				HString hstr;
				hr = dispInfo->get_GameDisplayName( hstr.GetAddressOf() );
