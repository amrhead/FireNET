...
bool CItem::SetAspectProfile( EEntityAspects aspect, uint8 profile )
{
	...
	if (aspect == eEA_Physics)
	{
		...
		case eIPhys_PhysicalizedStatic:
			{
				m_stats.physicalisedSlot = (m_stats.pickable && GetEntity()->IsSlotValid(eIGS_Aux0)) ? eIGS_Aux0 : eIGS_ThirdPerson;

				SEntityPhysicalizeParams params;
				params.type = PE_STATIC;
				params.nSlot = m_stats.physicalisedSlot;
		...
		case eIPhys_NotPhysicalized:
			{
				if(m_stats.physicalisedSlot != eIGS_Last)
				{
					...
					m_stats.physicalisedSlot = eIGS_Last;
				}
			}
			return true;
		}
...
void CItem::HideItem(bool hide)
{
	...
	if (pAttachmentManager)
	{
		...
		if (pAttachment)
		{
			...
			if(!hide && (m_stats.viewmode & eIVM_FirstPerson))
			{
...
//by AfroStalin, #inventory
CItem::SInventotyInfo CItem::GetInventoryInfo()
{
	SInventotyInfo info;
	info.itemName = m_sharedparams->params.item_name;
	info.itemClass = m_sharedparams->params.item_class;
	info.iconName = m_sharedparams->params.icon_name;
	info.itemDescription = m_sharedparams->params.item_description;
	info.canStack = m_sharedparams->params.can_stack;
	info.canQuick = m_sharedparams->params.can_quick;
