		...
		case eFE_Activate:
			{
				...
				EntityId entityId = pActInfo->pEntity ? pActInfo->pEntity->GetId() : 0;
				IGameObject* pGameObject = pGameFramework ? pGameFramework->GetGameObject(entityId): NULL;
				IAnimatedCharacter* pAnimChar = pGameObject ? (IAnimatedCharacter*) pGameObject->QueryExtension("AnimatedCharacter") : NULL;
				bool bSuccess = false;
				if(IsPortActive(pActInfo, EIP_Play))
				{
					if(pAnimChar && pAnimChar->GetActionController())
					{
						...
						IActionPtr pAction = new TPlayerAction((EPlayerActionPriority)priority, fragID, tagState);
...
public:
	...
	virtual void ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo )
	{
		...
		case eFE_Activate:
			IGameFramework* pGameFramework = g_pGame->GetIGameFramework();
			IGameObject* pGameObject = pActInfo->pEntity ? pGameFramework->GetGameObject(pActInfo->pEntity->GetId()) : NULL;
			...
			if(IsPortActive(pActInfo, EIP_Enslave) || IsPortActive(pActInfo, EIP_UnEnslave) )
			{
				...
				if(pAnimChar && pAnimChar->GetActionController())
				{
					...
					if(pSlaveAnimChar && pSlaveAnimChar->GetActionController())
					{
						...
						TagID scopeContext = pAnimChar->GetActionController()->GetContext().controllerDef.m_scopeContexts.Find(scopeContextName.c_str());

						if (scopeContext == SCOPE_CONTEXT_ID_INVALID)
						{
							if (const SMannequinPlayerParams* pParams = GetMannequinUserParams< SMannequinPlayerParams >(pAnimChar->GetActionController()->GetContext()))
							{
								scopeContext = pParams->contextIDs.SlaveChar;
							}
						}

						if (scopeContext == SCOPE_CONTEXT_ID_INVALID)
						{
							CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CFlowEnslaveCharacter::ProcessEvent: unrecognized ScopeContext: '%s'", scopeContextName.c_str());
