...
void CGameRulesObjective_CTF::Handle_RequestPickup( EntityId playerId )
{
	...
	if ((teamId == 1) || (teamId == 2))
	{
		...
		{
			Server_PhysicaliseFlag(pTeamInfo->m_flagId, false);
...
{
	FATAL_ASSERT(gEnv->bServer);
	Server_PhysicaliseFlag(pTeamInfo->m_flagId, true);
...
void CGameRulesObjective_CTF::Server_ResetFlag(STeamInfo *pTeamInfo, EntityId playerId)
{
	...
	Server_PhysicaliseFlag(flagId, false);
	Common_ResetFlag(flagId, playerId);
...
//-------------------------------------------------------------------------
void CGameRulesObjective_CTF::Server_PhysicaliseFlag(EntityId flagId, bool bEnable)
{
