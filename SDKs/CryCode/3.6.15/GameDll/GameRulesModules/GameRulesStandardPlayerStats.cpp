...
// Master server
#if defined(DEDICATED_SERVER)
			// Send new death to master server
			gClientEnv->pMasterServer->SendRequest("SvStats","death++",info.targetId);
#endif
...
// Master server
#if defined(DEDICATED_SERVER)
				// Send new kill to master server
				gClientEnv->pMasterServer->SendRequest("SvStats","kills++",info.shooterId);
#endif
