...
void CGameRulesStandardVictoryConditionsTeam::Init( XmlNodeRef xml )
{
	...
	SetFlagFromXML(xml, "canPlayMusicMoods", eSVCF_CanPlayMusicMoods, true, tempFlags);
...
void CGameRulesStandardVictoryConditionsTeam::ClUpdatedTimeLimit()
{
	...
	COMPILE_TIME_ASSERT(eSVC_size == ARRAY_COUNT(m_currentSoundTime));
...
//-------------------------------------------------------------------------
// TODO - this needs to be fixed for !timelimited modes (possible in a variant) which wouldn't play the start music! and announcement
void CGameRulesStandardVictoryConditionsTeam::UpdateTimeLimitSounds()
{
	if(gEnv->IsClient() && m_pGameRules->IsTimeLimited())
	{
		...
		CRY_ASSERT(m_currentSound >= 0 && m_currentSound <= eSVC_size);
		...
		if(timeRemaining <= m_currentSoundTime[csi])
		{
			...
			case eSVC_StartGame:
				{
					if (AreAnyFlagsSet(eSVCF_CanPlayMusicMoods))
					{
						CAudioSignalPlayer::JustPlay("Multiplayer_Music_Start");
					}
					csi++;
			...
			case eSVC_Halfway:
				{
					if (AreAllFlagsSet(eSVCF_PlayHalfwaySound | eSVCF_CanPlayMusicMoods) && (!AreAnyFlagsSet(eSVCF_HaveUpdatedTimeLimit) || AreAnyFlagsSet(eSVCF_PlayHalfwaySoundAfterUpdatingTimeLimit)))
					{
						CAudioSignalPlayer::JustPlay("Multiplayer_Music_Ambient");
					}
					else
					{
						CryLog("CGameRulesStandardVictoryConditionsTeam::UpdateTimeLimitSounds() not playing halfway sound.");
					}
					csi++;
			...
			case eSVC_1min:
				{
					...
					if (AreAnyFlagsSet(eSVCF_CanPlayMusicMoods))
					{
						CAudioSignalPlayer::JustPlay("Multiplayer_Music_60Secs");
					}
...
void CGameRulesStandardVictoryConditionsTeam::OnEndGame(int teamId, EGameOverReason reason, ESVC_DrawResolution winningResolution, EntityId killedEntity, EntityId shooterEntity)
{
	...
	if (gEnv->IsClient())
	{
		...
		TAudioSignalID musicSignal = INVALID_AUDIOSIGNAL_ID;
		EAnnouncementID announcementId = INVALID_ANNOUNCEMENT_ID;

		GetGameOverAnnouncementAndMusic(gameOverType, clientScore, opoonentScore, musicSignal, announcementId);
		...
		SHUDEventWrapper::OnGameEnd(teamId, clientScoreIsTop, reason, message_cached.c_str(), winningResolution, musicSignal, announcementId);
	}
...
//-------------------------------------------------------------------------
void CGameRulesStandardVictoryConditionsTeam::GetGameOverAnnouncementAndMusic(const EGameOverType gameOverType, const int clientScore, const int opponentScore, TAudioSignalID &musicSignal, EAnnouncementID& announcement)
{
	...
	if(gameOverType == EGOT_Win)
	{
		if(closeGame)
		{
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_WinClose");
			announcement = pAnnouncer->NameToID("WinClose");
		}
		else if (standardGame)
		{
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_WinClose");	// no WinStandard music!
			announcement = pAnnouncer->NameToID("WinStandard");
		}
		else
		{
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_WinBig");
	...
	else if(gameOverType == EGOT_Lose)
	{
		if(closeGame)
		{
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_LoseClose");
			announcement = pAnnouncer->NameToID("LoseClose");
		}
		else if (standardGame)
		{
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_LoseClose");	// no LoseStandard music!
			announcement = pAnnouncer->NameToID("LoseStandard");
		}
		else
		{
			musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_LoseBig");
			announcement = pAnnouncer->NameToID("LoseBig");
		}
	}
	else if(gameOverType == EGOT_Draw)
	{
		musicSignal = pGameAudio->GetSignalID("Multiplayer_Music_Draw");
...
void CGameRulesStandardVictoryConditionsTeam::SetCanPlayMusicMoods( bool bEnable )
{
	if (bEnable)
	{
		SetFlags(eSVCF_CanPlayMusicMoods);
	}
	else
	{
		ResetFlags(eSVCF_CanPlayMusicMoods);
	}
}
