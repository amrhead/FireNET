...
//Master server
	...
	CryLog("[CryMasterServer] Send gameId %d from player %s", playerId, sPlayerName);
	gClientEnv->pMasterServer->SendRequest("SetClientID",sPlayerName,playerId);
#endif
}
...
// Master server
#if defined(DEDICATED_SERVER)
		CryLogAlways("[Master Server] Register game server...");
