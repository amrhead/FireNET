...
void CGameRulesHoldObjectiveBase::SetMaterialDiffuseColor(IMaterial* pMaterial,const Vec3& diffuseColor) const
{
	if(pMaterial)
	{
		...
		{
			ColorF& matDiffuseColor = currShaderItem.m_pShaderResources->GetDiffuseColor();
			matDiffuseColor.r = diffuseColor.x;
			matDiffuseColor.g = diffuseColor.y;
			matDiffuseColor.b = diffuseColor.z;
...
void CGameRulesHoldObjectiveBase::SetMaterialDiffuseAlpha(IMaterial* pMaterial,float diffuseAlpha) const
{
	if(pMaterial)
	{
		...
		if(currShaderItem.m_pShaderResources)
		{
			currShaderItem.m_pShaderResources->GetDiffuseColor().a = diffuseAlpha;
		}
	}
}
