...
void CExplosionGameEffect::SpawnMaterialEffect(const SExplosionContainer &explosionContainer)
{
	...
	const Vec3 pos = params.pos;
	const float waterLevel = gEnv->p3DEngine->GetWaterLevel(&pos); 
	params.inWater = (buoyancy.waterPlane.origin.z > params.pos.z) && (waterLevel >= params.pos.z);
	...
	{
		if (gravity.len2() > 0.000001f)
			gravity.normalize();
		else
			gravity = Vec3(0.0f, 0.0f, -1.0f);

		params.dir[0] = gravity;
		params.normal = -gravity;
	}
