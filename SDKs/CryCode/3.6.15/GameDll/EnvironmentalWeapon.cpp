...
m_OwnerId(0) 
,m_State(EWS_Initial)
,m_OnClientHealthChangedFunc(NULL)
...
,m_pVelocityTracker(NULL)
,m_mass(100.0f)
,m_vehicleMeleeDamage(0.f)
,m_vehicleThrowDamage(0.f)
,m_vehicleThrowMinVelocity(5.f)
,m_vehicleThrowMaxVelocity(20.f)
			...
			if(m_pDamageZoneHelper)
			{
				...
				if(!m_debugCollisionPts_Geom.empty())
				{
					...
					for(std::vector<debugCollisionPt>::const_iterator iter = m_debugCollisionPts_Geom.begin(); iter != endIter; ++iter)
					{
						...
						gEnv->pRenderer->DrawLabelEx(iter->m_wCollisionPos, fFontSize, txtDrawColor, true, true, string().Format("%d",count).c_str());
						...
						if(iter->m_showDamageZoneHelper)
						{
							...
							gEnv->pRenderer->DrawLabelEx(dzhPos, fFontSize, txtDrawColor, true, true, string().Format("%d",count).c_str());
						}
					}
				}
