	...
	case eVE_PassengerExit:
		{
			...
			TAudioSwitchStateID nInOutStateID = INVALID_AUDIO_SWITCH_STATE_ID;
...
void CVehicleMovementBase::ExecuteTrigger(EVehicleMovementSound eSID)
{
	...
	const TAudioControlID id = m_audioControlIDs[eSID];
	if (id != INVALID_AUDIO_CONTROL_ID)
	{
		m_pIEntityAudioProxy->ExecuteTrigger(id, eLSM_None);
	}
}
...
//------------------------------------------------------------------------
const TAudioControlID& CVehicleMovementBase::GetAudioControlID(EVehicleMovementSound eSID)
{
