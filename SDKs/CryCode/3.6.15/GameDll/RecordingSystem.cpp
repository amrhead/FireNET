...
void CRecordingSystem::OnSetSubObjHideMask(IEntity *pEntity, int nSlot, uint64 nSubObjHideMask)
{
	EntityId entityId = pEntity->GetId();
	ClearBreakageSpawn(pEntity, entityId);
	...
	case ePSS_AudioAndTracers:
		{
			...
			// Stop all sound moods (e.g. low health mood)
			CGameAudio* pGameAudio = g_pGame->GetGameAudio();
			pGameAudio->ClearSoundMoods();
	...
	case ePSS_HandleParticles:
		{
			...
			// Remove all temporary particle emitters - 0.05ms
			gEnv->pParticleManager->DeleteEmitters(ePEGF_NotPartOfLevel);
...
void CRecordingSystem::ApplyEntitySpawn(const SRecording_EntitySpawn *entitySpawn, float time)
{
	...
	char sName[64];
	sprintf_s(sName, 64, "%s%s%d", entitySpawn->useOriginalClass?"Replay":"", params.pClass->GetName(), m_nReplayEntityNumber);
...
void CRecordingSystem::OnCreateEmitter(IParticleEmitter* pEmitter, QuatTS const& qLoc, const IParticleEffect* pEffect, uint uEmitterFlags)
{
	...
	if (m_replayEventGuard == 0 && IsRecording())
	{
		...
		{
			IParticleEffect* pEffectNonConst = const_cast<IParticleEffect*>(pEffect);
			if(!m_excludedParticleEffects.count(pEffectNonConst))
			{
				pEmitter->SetEmitterFlags(pEmitter->GetEmitterFlags() | ePEGF_NotPartOfLevel);
				if ((pEmitter->GetEmitterFlags()&ePEF_TemporaryEffect) == 0)
				{
					SRecording_ParticleCreated particle;
					particle.pParticleEmitter = pEmitter;
					particle.pParticleEffect = pEffectNonConst;
					particle.location = qLoc;
		...
		RecSysLogDebug(eRSD_Particles, "Emitter Created: %s (%p)", pEffect->GetName(), pEmitter);
	}
}
...
void CRecordingSystem::DebugDrawMemoryUsage()
{
	...
	if (g_pGameCVars->kc_memStats == 1)
	{
		...
		char temp[255];
		sprintf(temp, "First person buffer size: %" PRISIZE_T ", Frames: %d, Time: %f", m_pBufferFirstPerson->size(), numFPFrames, lastFPFrameTime - firstFPFrameTime);
...
const char* CRecordingSystem::CacheString(const char* name, eStringCache cache)
{
	...
	if( pBuffer != NULL )
	{
		const uint16 newLength = strlen(name);
		int offset = 0;
		while (true)
		{
...
IEntity * CRecordingSystem::SpawnCorpse( EntityId originalId, const Vec3 &position, const Quat &rotation, const Vec3 &scale )
{
	...
	char sName[64];
	sprintf_s(sName, 64, "ReplayCorpse%d", m_nReplayEntityNumber ++);
	params.sName = sName;
		...
		for(int g=0; g<numGroups; ++g)
		{
			...
			if(group->isTag("HitTypes"))
			{
				for(int c=0, num=group->getChildCount(); c<num; ++c)
				{
					...
					add = 0.f; child->getAttr("add", add);
					mul = 1.f; child->getAttr("mul", mul);
