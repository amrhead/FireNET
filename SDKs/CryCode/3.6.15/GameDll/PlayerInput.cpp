	...
	m_playerInVehicleAtFrameStart(false)
{
	...
	#define ADD_HANDLER(action, func) m_actionHandler.AddHandler(actions.action, &CPlayerInput::func)
		...
		//by AfroStalin #Inventory
		ADD_HANDLER(show_inventory, OnActionShowInventory);
...
//by AfroStalin #Inventory
bool CPlayerInput::OnActionShowInventory(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	if (activationMode==eAAM_OnRelease)
	{
		CHUDInventory* pHc =  g_pGame->GetHUDInventory();
		pHc->ShowInventory();
	}
	return false;
}
