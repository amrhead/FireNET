...
static const int		k_httpHeaderSize=512;											// should be large enough to hold the HTTP response and info on the payload length
...
void CDownloadMgr::InitiateShutdown()
{
	...
	if ( pInData && inDataLen )
	{
		...
		if ( newUsed > m_bufferSize)
		{
			...
			if (newUsed<=m_maxDownloadSize)
			{
				...
				newSize = max(newUsed, max(k_httpHeaderSize,expectedSize));
