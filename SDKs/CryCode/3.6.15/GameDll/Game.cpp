...
//Master server
#include "CryMasterServer/Nodes/MsEvents.h"
#include "CryMasterServer/System/Global.h"
//by AfroStalin
#include "MultiplayerEntities/ScriptBind_NetworkBuilding.h"
#include "UI/HUD/HUDInventory.h" //#Inventory
...
: m_pFramework(0),
	...
	// by AfroStalin
	m_pScriptBindNetworkBuilding(NULL),
	m_pHUDInventory(NULL), // #Inventory
	//
#ifdef DURANGO
...
bool CGame::Init(IGameFramework *pFramework)
{
  LOADING_TIME_PROFILE_SECTION(GetISystem());
	...
	m_pHUDInventory = new CHUDInventory();
	//
...
void CGame::InitScriptBinds()
{
	...
	//by AfroStalin
	m_pScriptBindNetworkBuilding = new CScriptBind_NetworkBuilding(m_pFramework->GetISystem(), m_pFramework);
...
void CGame::ReleaseScriptBinds()
{
	...
	//by AfroStalin
	SAFE_DELETE(m_pScriptBindNetworkBuilding);
}
...
void CGame::GetMemoryStatistics(ICrySizer * s)
{
	...
	//by AfroStalin
	s->Add(*m_pScriptBindNetworkBuilding);
		...
		case ESYSTEM_EVENT_PLM_ON_CONSTRAINED:
			{
				m_wasGamePausedBeforePLMForcedPause = gEnv->pGame->GetIGameFramework()->IsGamePaused();
				gEnv->pGame->GetIGameFramework()->PauseGame(true, false);
				// Pause music system here (sound system will be paused by the game pause)
				gEnv->pMusicSystem->Pause(true);
			}
			break;
