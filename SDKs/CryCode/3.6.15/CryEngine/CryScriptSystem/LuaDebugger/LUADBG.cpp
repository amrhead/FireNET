...
bool CLUADbg::LoadFile(const char *pszFile, bool bForceReload)
{
	...
	// set filename in window title
	{
		...
		_snprintf_s(str, sizeof(str), _TRUNCATE, "Lua Debugger [%s]",pszFile);
		SetWindowText(str);
	}
...
HTREEITEM CLUADbg::AddVariableToTree(const char *sName, ScriptVarType type, HTREEITEM hParent)
{
	...
	if (type == svtString || type == svtNumber || type == svtBool || type==svtPointer || type==svtUserData)
	{
		...
		{
			if (type == svtString)
				_snprintf_s(szBuf, sizeof(szBuf), _TRUNCATE, "%s %s = \"%s\"", szType, sName, szContent);
			else
				_snprintf_s(szBuf, sizeof(szBuf), _TRUNCATE, "%s %s = %s", szType, sName, szContent);
		}
		else
			_snprintf_s(szBuf, sizeof(szBuf), _TRUNCATE, "%s %s = (Unknown)", szType, sName);
	}
	else
		_snprintf_s(szBuf, sizeof(szBuf), _TRUNCATE, "%s %s", szType, sName);
...
{
	char szBuf[32];
	sprintf_s(szBuf, sizeof(szBuf), "[%i]", nIdx);
...
void CLUADbg::LoadBreakpoints( const char *filename )
{
	...
	if (file)
	{
		...
		while (!feof(file))
		{
			const int result = fscanf(file,"%d:%d:%s",&nLine,&tracepoint,str);
			if (result == 3)
			{
