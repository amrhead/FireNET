...
void CEntity::Serialize( TSerialize ser, int nFlags )
{
	...
	if (nFlags & ENTITY_SERIALIZE_PROXIES)
	{
		...
		//Note: This is not really needed, the only thing of use it is doing is saving attachment state, which in any case is always reset by the game
#if !defined(GAME_IS_CRYSIS3)
		// If we have an AI, serialise the character. AI object itself may not exist yet.
		if(m_aiObjectID)
		{
...
void CEntity::FreeSlot( int nSlot )
{
	CRenderProxy * pRenderProxy = GetRenderProxy();
	if (pRenderProxy)
		return pRenderProxy->FreeSlot(nSlot);
}
...
void CEntity::SetSlotLocalTM( int nSlot,const Matrix34 &localTM,int nWhyFlags )
{
	...
	CRenderProxy * pRenderProxy = GetRenderProxy();
	if (pRenderProxy)
		return pRenderProxy->SetSlotLocalTM(nSlot,localTM);
}
...
void CEntity::SetSlotMaterial( int nSlot,IMaterial *pMaterial )
{
	CRenderProxy * pRenderProxy = GetRenderProxy();
	if (pRenderProxy)
		return pRenderProxy->SetSlotMaterial(nSlot,pMaterial);
}
...
void CEntity::SetSlotFlags( int nSlot,uint32 nFlags )
{
	CRenderProxy * pRenderProxy = GetRenderProxy();
	if (pRenderProxy)
		return pRenderProxy->SetSlotFlags(nSlot,nFlags);
}
...
{
	return LoadLightImpl(nSlot, pLight, false);
}

int CEntity::LoadLightImpl( int nSlot,CDLight *pLight,bool isCheapLight )
{
	if (!GetRenderProxy())
		CreateProxy(ENTITY_PROXY_RENDER);
	...
	if (m_bIsCheapLight)
	{
		// cheap light can only pass this path once (we otherwise re-add this light each time we pass here) - something went wrong
		CryWarning(VALIDATOR_MODULE_ENTITYSYSTEM, VALIDATOR_ERROR_DBGBRK, "!Trying to dynamically load light properties for cheap light \"%s\"! "
			"Check ForceDisableCheapLight in light properties to prevent this.", m_szName.c_str());
	}
#endif
	...
	if (isCheapLight)
	{
		const CEntityLayer* pLayer = g_pIEntitySystem->GetLayerForEntity(GetId());
		if (pLayer)
			layerId = pLayer->GetId();
	}

	m_bIsCheapLight = isCheapLight;
	return GetRenderProxy()->LoadLight( nSlot,pLight,isCheapLight,layerId );
}
...
bool CEntity::UpdateLightClipBounds(CDLight &light)
{
	for (IEntityLink *pLink = m_pEntityLinks; pLink; pLink = pLink->next)
	{
		...
		if ( pLinkedEntity )
		{
			bool lightShape = _stricmp(pLinkedEntity->GetClass()->GetName(), "LightShape") == 0;
			bool lightBox = _stricmp(pLinkedEntity->GetClass()->GetName(), "LightBox") == 0;
			if (pLinkedEntity && ( lightBox || lightShape ) )
			{
				IEntityAreaProxy *pArea = (IEntityAreaProxy*)pLinkedEntity->GetProxy(ENTITY_PROXY_AREA);
				if (pArea)
				{
          Vec3 scale(1.0f,1.0f,1.0f);
          Matrix34 clipBox = Matrix34::CreateIdentity();

					if ( lightBox )
					{
						Vec3 min, max;
						pArea->GetBox(min, max);
						clipBox = Matrix34::CreateScale(max - min, min);
						scale = pLinkedEntity->GetScale();
					}

					clipBox.ScaleColumn(scale);
          clipBox.m03 *= scale.x;
          clipBox.m13 *= scale.y;
          clipBox.m23 *= scale.z;
          clipBox = (Matrix33)pLinkedEntity->GetWorldRotation() * clipBox;
          clipBox.AddTranslation(pLinkedEntity->GetWorldPos());
          light.m_ClipBox = clipBox;
					return true;
				}
			}
		}
...
//////////////////////////////////////////////////////////////////////////
int CEntity::LoadLightShape( int nSlot,const char *sFilename, const uint32 nLSFlags )
{
	if (!GetRenderProxy())
		CreateProxy(ENTITY_PROXY_RENDER);
	return GetRenderProxy()->LoadLightShape( nSlot,sFilename, nLSFlags );
}
