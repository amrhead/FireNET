...
void CRenderProxy::ExpandCompoundSlot0()
{
	...
	gEnv->p3DEngine->SyncOctreeUpdate(this);
...
//				const char* eventName = (pAnimEvent ? pAnimEvent->m_EventName : 0);
				...
				if (!CheckFlags(FLAG_HIDDEN) && pSkeletonAnim && pAnimEvent->m_EventName && stricmp(pAnimEvent->m_EventName, "effect") == 0 && pAnimEvent->m_CustomParameter)
				{
					for (Slots::iterator it = m_slots.begin(),endit = m_slots.end(); it != endit; ++it) 
					{
						...
						if (pSlot && pSlot->pCharacter == pCharacter)
							pSlot->pCharacter->SpawnSkeletonEffect(pAnimEvent->m_AnimID, pAnimEvent->m_AnimPathName, pAnimEvent->m_CustomParameter, pAnimEvent->m_BonePathName, pAnimEvent->m_vOffset, pAnimEvent->m_vDir, QuatTS(pSlot->m_worldTM));
					}
				}
			}
		}
  ...
  case ENTITY_EVENT_PREPHYSICSUPDATE:
    // Disable the octree updates because the slot geometry changes will cause a storm of
    // updates that run in lockstep 
    bool wasEnabled = gEnv->p3DEngine->ToggelAsyncOctreeUpdates(false);
    ...
    gEnv->p3DEngine->ToggelAsyncOctreeUpdates(wasEnabled);
...
SPU_INDIRECT(AsyncOctreeUpdate(MM))
...
//////////////////////////////////////////////////////////////////////////
int CRenderProxy::LoadLight( int nSlot,CDLight *pLight,bool isCheapLight,uint16 layerId )
{
	assert(pLight);
	CEntityObject *pSlot = GetOrMakeSlot(nSlot);
	...
	if (isCheapLight)
	{
		if (pSlot->pLight)
		{
			CDLight* pDLight = &pSlot->pLight->GetLightProperties();
			pSlot->pLight->SetCheapLight(m_pEntity->GetGuid());
			pDLight->m_sName = "CheapLight";
		}
		pSlot->pLight = 0;
	}
...
//////////////////////////////////////////////////////////////////////////
int CRenderProxy::LoadLightShape( int nSlot,const char *sFilename, const uint32 nLSFlags )
{
	CEntityObject *pSlot = GetOrMakeSlot(nSlot);

	pSlot->ReleaseObjects();
	ILightShapeRenderNode *pLightShape = (ILightShapeRenderNode*)GetI3DEngine()->CreateRenderNode( eERType_LightShape );
	pSlot->pChildRenderNode = pLightShape;
	pLightShape->LoadLightShape( sFilename, nLSFlags );
...
//////////////////////////////////////////////////////////////////////////
SPU_INDIRECT(AsyncOctreeUpdate(M))
...
//////////////////////////////////////////////////////////////////////////
void CRenderProxy::SetSubObjHideMask( int nSlot,uint64 nSubObjHideMask )
{
	if (IsSlotValid(nSlot))
	{
		CEntityObject *pSlot = Slot(nSlot);		
...
//////////////////////////////////////////////////////////////////////////
uint64 CRenderProxy::GetSubObjHideMask( int nSlot ) const
{
	if (IsSlotValid(nSlot))
	{
		CEntityObject *pSlot = Slot(nSlot);
...
//////////////////////////////////////////////////////////////////////////
SPU_INDIRECT(CRenderMesh_Render(ML),RenderContent(ML),InvokeShadowMapRenderJobs(ML),AsyncOctreeUpdate(ML))
...
//////////////////////////////////////////////////////////////////////////
SPU_INDIRECT(RenderContent(M),InvokeShadowMapRenderJobs(M),AsyncOctreeUpdate(M))
