...
{
	ICharacterInstance* pCharacterInstance = const_cast<CEntity*>(pParent)->GetCharacter(0);
...
Matrix34 CCharacterBoneAttachmentManager::GetNodeWorldTM(const CEntity* pChild, const CEntity* pParent) const
{
	...
	if (findIter != m_attachments.end())
	{
		...
		ICharacterInstance* pCharacterInstance = const_cast<CEntity*>(binding.pParent)->GetCharacter(0);
		if (pCharacterInstance)
		{
