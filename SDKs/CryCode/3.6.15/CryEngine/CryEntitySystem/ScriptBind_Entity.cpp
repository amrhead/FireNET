...
#define SCRIPT_REG_CLASSNAME &CScriptBind_Entity::
	...
	SCRIPT_REG_TEMPLFUNC(LoadLightShape, "nSlot,sLightShapeFile,nLSFlags");
...
int CScriptBind_Entity::LoadLight(IFunctionHandler *pH,int nSlot,SmartScriptTable table)
{
	...
	CDLight light;
	bool isCheapLight = false;
	if (ParseLightParams(table, light, &isCheapLight))
	{
		pEntity->UpdateLightClipBounds(light);
		nSlot = pEntity->LoadLightImpl(nSlot, &light, isCheapLight);
		if (nSlot < 0 || isCheapLight)
			return pH->EndFunction();
	}
...
//////////////////////////////////////////////////////////////////////////
int CScriptBind_Entity::LoadLightShape( IFunctionHandler* pH, int nSlot, const char * sLightShapeFile, const uint32 nLSFlags)
{
	GET_ENTITY;

	nSlot = pEntity->LoadLightShape( nSlot, sLightShapeFile, nLSFlags);
	return( pH->EndFunction( nSlot ) );
}
...
int CScriptBind_Entity::SetAudioEnvironmentID(IFunctionHandler* pH, ScriptHandle const hAudioEnvironmentID)
{
	...
	if (pIEntityAudioProxy)
	{
		pIEntityAudioProxy->SetEnvironmentID(HandleToInt<TAudioEnvironmentID>(hAudioEnvironmentID));
	}
...
bool CScriptBind_Entity::ParseLightParams(IScriptTable *pLightTable, CDLight &light, bool* pIsCheapLight)
{
	...
	bool isCheapLight = false;
	chain.GetValue("cheapLight", isCheapLight);
	...
	if (chain.GetValue("disable_x360_opto", flag) && flag)
		light.m_Flags |= DLF_DISABLE_X360_MSAA;
  ...
  if (chain.GetValue("projector_texture", projectorTexture))
  {
	...
	if (pIsCheapLight)
	{
		...
		isCheapLight &= !gEnv->IsEditor();
#endif
		*pIsCheapLight = isCheapLight;
	}
