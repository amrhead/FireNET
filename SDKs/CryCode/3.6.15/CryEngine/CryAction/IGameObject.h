...
public:
	...
	bool Sync()
	{
		...
		if (IGameObject * pGameObject = gEnv->pGame->GetIGameFramework()->GetGameObject(m_id))
		{
			...
			{
				sprintf(msg, "Game object extension with base %.8x for entity %s for RMI %s not found", (uint32)(TRUNCATE_PTR)m_pRMI->pBase, pGameObject->GetEntity()->GetName(), m_pRMI->pMsgDef->description);
				GameWarning("%s", msg);
			}
		}
		else
