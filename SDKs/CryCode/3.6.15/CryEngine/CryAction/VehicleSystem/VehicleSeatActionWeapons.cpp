...
IEntity* CVehicleSeatActionWeapons::SpawnWeapon(SVehicleWeapon& weapon, IEntity* pVehicleEntity, const string& partName, int weaponIndex)
{
	...
	if ((isServer || bClientOnly) && static_cast<CVehicle*>(m_pVehicle)->SpawnAndDeleteEntities())        
	{
		...
		if (pWeaponEntity)
		{
			CVehicle *pCVehicle=static_cast<CVehicle *>(m_pVehicle);
			...
			EntityId lastId = pCVehicle->GetLastCreatedWeaponId();
			if (lastId && !bClientOnly)
			{
				EntityId weaponEntityId = pWeaponEntity->GetId();
				IGameObject *pGameObject=CCryAction::GetCryAction()->GetGameObject(weaponEntityId);
				if (pGameObject)
				{
					pGameObject->SetNetworkParent(lastId);
					weapon.networkParentId = lastId;

					SVehicleWeapon* pParentVehicleWeapon = pCVehicle->GetVehicleWeaponAllSeats(lastId);
					if(pParentVehicleWeapon)
					{
						pParentVehicleWeapon->networkChildId = weaponEntityId;
					}
				}
			}
			...
			if (!bClientOnly)
			{
				...
				weapon.networkChildId = m_pVehicle->GetEntityId();
			}
