...
bool CVehicle::Init( IGameObject * pGameObject )
{
  ...
  InitPaint(vehicleParams);
	...
	pEntity->AddFlags(ENTITY_FLAG_CASTSHADOW|ENTITY_FLAG_ON_RADAR|ENTITY_FLAG_CUSTOM_VIEWDIST_RATIO|ENTITY_FLAG_TRIGGER_AREAS);
	//////////////////////////////////////////////////////////////////////////
...
void CVehicle::OnCollision(EventPhysCollision* pCollision)
{
  ...
  IEntity* pCollider = idx ? pE1 : pE2;
	...
	// C2 doesn't use this event, so avoid the calls to multiple listeners
	SVehicleEventParams eventParams;
...
bool CVehicle::IsProbablyDistant() const
{
	...
	if(IActor *pActor = CCryAction::GetCryAction()->GetClientActor())
	{
		...
		const float FAR_AWAY_DISTANCE = 150.0f, FAR_AWAY_DISTANCE_SQ = FAR_AWAY_DISTANCE * FAR_AWAY_DISTANCE;

		return pActor->GetEntity()->GetWorldPos().GetSquaredDistance(GetEntity()->GetWorldPos()) > FAR_AWAY_DISTANCE_SQ;
	}
	else
	{
		return false;
