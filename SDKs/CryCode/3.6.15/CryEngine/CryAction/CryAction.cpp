...
#include "AI/CommunicationVoiceLibrary.h"
#include "AI/CommunicationVoiceTestManager.h"
...
: m_paused(false),
  ...
  m_nextFrameCommand(0),
	...
	m_pAIProxyManager(0),
	m_pSegmentedWorld(0),
	m_pCommunicationVoiceLibrary(0),
...
void CCryAction::MapCmd( IConsoleCmdArgs* args )
{
  ...
  LOADING_TIME_PROFILE_SECTION;
	...
	uint32 flags = eGSF_ImmersiveMultiplayer;	// marcio: Immersiveness always enabled for Crysis 2
...
void CCryAction::LegacyStatusCmd( IConsoleCmdArgs* args )
{
	...
	else
	{
		...
		for (TServerChannelMap::iterator iter = pChannelMap->begin(); iter != pChannelMap->end(); ++iter)
		{
			...
			CryLogAlways("name: %s  entID:%d id: %d  ip: %s  ping: %d  state: %d profile: %d", name, entId, iter->first, ip, ping, state, profileId);
		}
	}
}
		...
		if (pArgs->GetArgCount() > 1)
		{
			...
			else
			{
				if (_stricmp(pArgs->GetArg(1), "id") == 0)
				{
					if (pArgs->GetArgCount() == 3)
					{
						sscanf_s(pArgs->GetArg(2), "%lld", &id);
					}
				}
				else
				{
...
void CCryAction::rcon_startserver(IConsoleCmdArgs* args)
{
	...
	uint16 port = RCON_DEFAULT_PORT; string rcon_password;
...
bool CCryAction::Init(SSystemInitParams &startupParams)
{
	...
	if (!startupParams.pSystem)
	{
		...
		m_systemDll = 0; // Since we used GetModuleHandle, don't try to free it later
#endif
...
//	gEnv->pFrameProfileSystem->Enable( true, false );
	...
	{
		m_pCommunicationVoiceLibrary = new CommunicationVoiceLibrary();
		m_pCommunicationVoiceLibrary->ScanFolder("Libs/Communication/Voice");
		InlineInitializationProcessing("CCryAction::Init m_pCommunicationVoiceLibrary");
	}
...
//	InlineInitializationProcessing("CCryAction::Init m_pCharacterPartsManager");
	...
	if(gEnv->pAISystem)
	{
		...
		RegisterActionBehaviorTreeNodes(gEnv->pAISystem->GetIBehaviorTreeManager()->GetNodeFactory());
	}
...
void CCryAction::Shutdown()
{
	...
	SAFE_DELETE(m_pCommunicationVoiceLibrary)
	...
	// Shutdown pFlashUI after shutdown and deleting FlowSystem
	// since FlowSystem will hold UIFlowNodes until deletion
...
{
	m_pCommunicationVoiceLibrary->InitEditor(pGameToEditor);
...
{
	gEnv->p3DEngine->SyncOctreeUpdate(NULL);
  ...
  if (!pClientActor)
	...
	if (bForceImmediate)
	{
		...
		if(gEnv->pStatoscope)
		{
			...
			switch(reason)
			{
			case eSGR_Command: s = "Command"; break;
			case eSGR_FlowGraph: s = "FlowGraph"; break;
			case eSGR_LevelStart: s = "LevelStart"; break;
			case eSGR_QuickSave: s = "QuickSave"; break;
			default: s = "Unknown Reason"; break;
			}
...
void CCryAction::OnEditorSetGameMode( int iMode )
{
	...
	if (iMode==0)
	{
		...
		{
			gEnv->pMusicSystem->EndTheme(EThemeFade_FadeOut);
	...
	if(m_pCommunicationVoiceLibrary)
	{
		m_pCommunicationVoiceLibrary->Reset();
	}
}
...
void CCryAction::InitCVars()
{
  ...
  REGISTER_STRING("sv_requireinputdevice", "dontcare", VF_DUMPTODISK | VF_REQUIRE_LEVEL_RELOAD, "Which input devices to require at connection (dontcare, none, gamepad, keyboard)");
	...
	if (gEnv->IsDedicated())
		defaultGameRules = "TeamInstantAction";
...
void CCryAction::InitCommands()
{
  ...
  REGISTER_COMMAND( "kickid", KickPlayerByIdCmd, 0, "Kicks player from game");
	...
	REGISTER_COMMAND( "ai_reload_voicelib", ReloadVoiceLibraryCmd, VF_CHEAT,"" );
	REGISTER_COMMAND( "ai_voiceTest", TestVoiceLibraryCmd, VF_CHEAT,"" );
	REGISTER_COMMAND( "ai_voiceTestStop", TestVoiceLibraryStopCmd, VF_CHEAT,"" );
}
	...
	else
	{
		if (StartedGameContext())
			if (ILevel * pLevel = GetILevelSystem()->GetCurrentLevel())
				if (ILevelInfo * pLevelInfo = pLevel->GetLevelInfo())
				{
					sprintf(pPath,"%s/",PathUtil::GetGameFolder().c_str());
					strcpy(&pPath[strlen(pPath)],pLevelInfo->GetPath());
...
void CCryAction::ReloadVoiceLibraryCmd(IConsoleCmdArgs* args)
{
	CCryAction::GetCryAction()->GetCommunicationVoiceLibrary()->Reload();
}

void CCryAction::TestVoiceLibraryCmd(IConsoleCmdArgs* args)
{
	if (args->GetArgCount() < 2)
	{
		GameWarning("<VoiceTest> Expecting actorName as parameter.");
	...
	IAIActor* actor = 0;
	IEntity* entity = 0;
	if (entity = gEnv->pEntitySystem->FindEntityByName(args->GetArg(1)))
		if (IAIObject* aiObject = entity->GetAI())
			actor = aiObject->CastToIAIActor();

	if (!actor)
	{
		GameWarning("<VoiceTest> Could not find entity named '%s' or it's not an actor.", args->GetArg(1));
	...
	if (args->GetArgCount() > 2)
		libName = args->GetArg(2);

	CCryAction::GetCryAction()->GetCommunicationVoiceLibrary()->GetTestManager().StartFor(entity->GetId(), libName);
}

void CCryAction::TestVoiceLibraryStopCmd(IConsoleCmdArgs* args)
{
	if (args->GetArgCount() < 2)
	{
		GameWarning("<VoiceTest> Expecting actorName as parameter.");
	...
	CCryAction::GetCryAction()->GetCommunicationVoiceLibrary()->GetTestManager().Stop(entity->GetId());
}
...
#define CHILD_STATISTICS(x) if (x) (x)->GetMemoryStatistics(s)
	...
	s->AddObject(m_pCommunicationVoiceLibrary);
...
ICommunicationVoiceLibrary* CCryAction::GetICommunicationVoiceLibrary() const
{
	return m_pCommunicationVoiceLibrary;
}
...
void CCryAction::StartNetworkStallTicker(bool includeMinimalUpdate)
{
	...
	if (gEnv->bMultiplayer)
	{
		...
		{
			CRY_ASSERT(m_pNetworkStallTickerThread == NULL);
