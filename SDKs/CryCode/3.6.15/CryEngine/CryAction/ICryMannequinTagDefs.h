...
//
////////////////////////////////////////////////////////////////////////////
#ifndef __I_CRY_MANNEQUIN_TAG_DEFS_H__
#define __I_CRY_MANNEQUIN_TAG_DEFS_H__
...
static const uint32 TAGSTATE_MAX_BYTES = 12;
...
struct STagState
{
	...
	template <typename T>
	void GetToInteger(T &value)
	{
		COMPILE_TIME_ASSERT(sizeof(T) == NUM_BYTES);

#ifdef NEED_ENDIAN_SWAP
		uint8 *pOut = ((uint8 *)&value)+NUM_BYTES;
		for (uint32 i=0; i<NUM_BYTES; i++)
		{
			*(--pOut) = state[i];
		}
#else // (!NEED_ENDIAN_SWAP)
		memcpy(&value, state, NUM_BYTES);
#endif // (!NEED_ENDIAN_SWAP)
	}
...
public:
	...
	//-----------------------------------------------------------------------------
	uint32 AssignBits(STagDefData &tagDefData, const STagStateBase usedTags = STagStateBase(0, 0)) const
	{
		...
		for (uint32 curByte=0; (curByte<TAGSTATE_MAX_BYTES) && (tagsMapped<totalTagsToMap); curByte++)
		{
			uint8 curBit = 0;
		...
		if (tagsMapped<totalTagsToMap)
		{
			CryWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "!Could not assign bits for %s, do you really need to increase TAGSTATE_MAX_BYTES?", m_filename.c_str());
		}
	...
	bool TagListToFlags(const char *tagList, STagStateBase tagState) const
	{
		bool success = true;
		tagState.Clear();
	...
	template <typename T>
	void IntegerFlagsToTagList(const T &tagStateInteger, char *tagList, uint32 length) const
	{
		STagState<sizeof(T)> tagState;
		tagState.SetFromInteger(tagStateInteger);

		FlagsToTagList(tagState, tagList, length);
	}

	void FlagsToTagList(const STagStateBase &tagState, char *tagList, uint32 length) const
	{
		...
		for (TagID i=0; i<numTags; i++)
		{
			if (m_defData.tagMasks[i].mask && IsSetInternal(tagState, i))
			{
				...
				if (!isFirst)
				{
					CRY_ASSERT(length - tagListLen > 0);
					strncat(tagList, "+", length - tagListLen);
				        ++tagListLen;
					//					retBuffer += "+";
				}
				isFirst = false;
				CRY_ASSERT(length - tagListLen > 0);
				strncat(tagList, tagName, length - tagListLen);
				tagListLen += strlen(tagName);
				//				retBuffer += tagName;
			}
		}
		uint32 len = static_cast<uint32>(tagListLen);
		uint32 end = min(len, length-1);
		tagList[end] = '\0';
		//		strncat(tagList, "\0", length);
		//		retBuffer += "\0";
...
#endif //__I_CRY_MANNEQUIN_TAG_DEFS_H__
