...
void CGameSerialize::SaveEngineSystems( SSaveEnvironment &savEnv )
{
	...
	// SoundSystem data
	REINST("serialize the audiosystem?");
	/*if (gEnv->pAudioSystem)
		gEnv->pAudioSystem->Serialize(savEnv.m_pSaveGame->AddSection(SAVEGAME_SOUNDSYSTEM_SECTION));
	savEnv.m_checkpoint.Check("SoundSystem");*/
...
void CGameSerialize::LoadEngineSystems( SLoadEnvironment &loadEnv )
{
	...
	REINST("serialize the audiosystem?");
	//ISoundSystem* pSoundystem = gEnv->pAudioSystem;
	//if (pSoundystem)
	//{
	//	pSoundystem->Silence(false, false);
	//	pSoundystem->Update(eSoundUpdateMode_All); // call one update so it can remove some sounds
	//	loadEnv.m_pSer = loadEnv.m_pLoadGame->GetSection(SAVEGAME_SOUNDSYSTEM_SECTION);

	//	if (loadEnv.m_pSer.get())
	//		pSoundystem->Serialize(*loadEnv.m_pSer);
	//	else
	//		GameWarning("Unable to open section %s", SAVEGAME_SOUNDSYSTEM_SECTION);
	//}
	//loadEnv.m_checkpoint.Check("SoundSystem");
