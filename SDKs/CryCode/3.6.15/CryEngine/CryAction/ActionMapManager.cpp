...
//------------------------------------------------------------------------
void CActionMapManager::AddExtraActionListener(IActionListener* pExtraActionListener)
{
	stl::push_back_unique(m_ExtraActionListeners, pExtraActionListener);
}

//------------------------------------------------------------------------
void CActionMapManager::RemoveExtraActionListener(IActionListener* pExtraActionListener)
{
...
bool CActionMapManager::HandleAcceptedEvents(const SInputEvent &event, TBindPriorityList& priorityList)
{
	...
	for (; itBind != itBindEnd; ++itBind)
	{
		...
		const CActionMap* pActionMap = pBindData->m_pActionMap;
		CRY_ASSERT(pActionMap != NULL);
