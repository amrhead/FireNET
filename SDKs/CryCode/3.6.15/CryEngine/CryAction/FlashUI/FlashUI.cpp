...
string CreateDisplayName( const char *format,... )
{
	...
	char result[1024];
	vsnprintf_s(result,1024, 1023,  format, args);
	va_end(args);
	return result;
}
...
CFlashUI::~CFlashUI()
{
	for ( TUIFlowNodes::iterator it = m_UINodes.begin(); it != m_UINodes.end(); ++it )
		delete *it;
...
void CFlashUI::Update(float fDeltaTime)
{
	...
	if (gEnv && gEnv->pSystem)
	{
		...
		if (CV_gfx_ampserver == 1)
		{
			if (!ampEnabled)
			{
				gEnv->pSystem->GFxAmpEnable(true);
				ampEnabled = true;
			}
			gEnv->pSystem->GFxAmpAdvanceFrame();
		}
		else if (ampEnabled)
		{
			gEnv->pSystem->GFxAmpEnable(false);
			ampEnabled = false;
		}
	}
#endif
	...
	case ESYSTEM_EVENT_LEVEL_LOAD_END:
		{
			...
			PreloadTextures(pLevel && pLevel->GetLevelInfo() ? pLevel->GetLevelInfo()->GetName() : "");
...
void CFlashUI::UpdateFG()
{
	...
	CRY_ASSERT_MESSAGE( !isUpdating, "try to update UIAction FlowGrapsh within update loop!" );
	if (!isUpdating)
	{
		isUpdating = true;
		int loops = 0;
...
//-------------------------------------------------------------------
void CFlashUI::DispatchControllerEvent( IUIElement::EControllerInputEvent event, IUIElement::EControllerInputState state )
{
	...
	for (TSortedElementList::const_reverse_iterator iter = sortedElements.rbegin(); iter != sortedElements.rend(); ++iter)
	{
		...
		{
			iter->second->SendControllerEvent( event, state );
			if ( iter->second->HasFlag( IUIElement::eFUI_EVENTS_EXCLUSIVE) )
				return;
		}
	}
...
void CFlashUI::CreateMouseClick(  IUIElement::EControllerInputState state  )
{
	...
	switch ( state )
	{
	case IUIElement::eCIS_OnPress: evt = SFlashCursorEvent::eCursorPressed;
		break;
	case IUIElement::eCIS_OnRelease: evt = SFlashCursorEvent::eCursorReleased;
		break;
	default: 
		return;
	}
...
bool CFlashUI::PreloadTexture(const char* pFileName)
{
		...
		|| sFile.find(".tif") == sFile.length() - 4)
	{
		if (gEnv && gEnv->pRenderer)
		{
			...
			ITexture* pTexture = gEnv->pRenderer->EF_LoadTexture(sFile.c_str(), FT_DONT_STREAM | FT_DONT_RESIZE | FT_NOMIPS);
...
TUIEventSystemMap* CFlashUI::GetEventSystemMap( IUIEventSystem::EEventSystemType eType )
{
	...
	switch (eType)
	{
	case IUIEventSystem::eEST_SYSTEM_TO_UI: pMap = &m_eventSystemsSysToUi; break;
	case IUIEventSystem::eEST_UI_TO_SYSTEM: pMap = &m_eventSystemsUiToSys; break;
	default: CRY_ASSERT_MESSAGE( false, "Invalid IUIEventSystem::EEventSystemType type" );
	}
	return pMap;
}
...
void CFlashUI::LogUIAction( ELogEventLevel level, const char *format, ... )
{
	...
	va_start( args, format );
	char tmp[1024];
	vsnprintf_s( tmp, 1024, 1023, format, args );
	va_end( args );

	if ( gEnv->IsEditor() && CV_gfx_uiaction_log )
	{
...
struct SStackInfo
{
	...
	SStackInfo(float t = 0)  : time(t), index(-1), id(-1) {}
	SStackInfo(const char* str, float t) : info(str), time(t), index(-1), id(-1) {}
