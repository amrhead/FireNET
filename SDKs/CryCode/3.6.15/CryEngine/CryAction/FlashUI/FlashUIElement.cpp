...
void CFlashUIElement::SendControllerEvent( EControllerInputEvent event, EControllerInputState state )
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_ACTION);
	...
	if ( m_pFlashplayer->IsAvailable("cry_onController") )
	{
		...
		SFlashVarValue args[] = { SFlashVarValue( (int) event ), SFlashVarValue( state == eCIS_OnRelease ) };
		m_pFlashplayer->Invoke( "cry_onController", args, 2 );
		m_pFlashplayer->Advance(0);
	}
	ForceLazyUpdateInl();
}
