...
bool CActionScope::PlayPendingProc(uint32 layer)
{
	...
	if ((sequencer.pos < sequencer.sequence.size()) || ((sequencer.flags & eSF_BlendingOut) != 0))
	{
		...
		{
			const SProcClip& procClip = sequencer.sequence[sequencer.pos];
			...
			InstallProceduralClip(procClip.procedural, layer, sequencer.blend, duration);
			ClipInstalled(procClip.part);
