...
float AppendLayers(SFragmentData &fragmentData, const CFragment &fragment, uint8 partID, const IAnimationSet *animSet, float startTime, float startOffset, bool isBlend, bool &isOneShot)
{
	...
	SProcClip nullProc;
	nullProc.procedural.animRef.SetByString(NULL);
	nullProc.procedural.dataCRC.SetByString(NULL);
	nullProc.procedural.type = 0;
	nullProc.procedural.dataString.reset();
	nullProc.procedural.dataString2.reset();
	...
	for (uint32 i=0; i<numPLayers; i++)
	{
		...
		{
			const bool firstClip = (k==0);
			const SProcClip *sourceClip = (k<fragLength) ? &fragment.m_procLayers[i][k] : &nullProc;
			SProcClip &procClip = fragmentData.procLayers[i][startIdx+k];

			procClip.procedural = sourceClip->procedural;
	...
	for (uint32 i=numPLayers; i<fragmentData.procLayers.size(); i++)
	{
		...
		{
			//--- Should we merge over the existing final blend or append to it?
			SProcClip &procClip = fragmentData.procLayers[i][oldLength-1];
			...
			else
			{
				SProcClip newClip;
				newClip.blend.exitTime = startTime;
				newClip.blendPart = procClip.part;
				newClip.part = partID;
				fragmentData.procLayers[i].push_back(newClip);
			}
		}
	}
...
bool CAnimationDatabase::DeleteEntry(FragmentID fragmentID, const SFragTagState &tags, uint32 optionIdx)
{
	...
	if (fragmentID < numFragmentTypes)
	{
		...
		else
		{
			...
			{
				CryWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "AnimDatabase: Invalid option index: %d for fragment id: %d passed to %s", optionIdx, fragmentID, m_filename.c_str());
				return false;
			}
		}
		...
		for (uint32 c=0; c<numClips; c++)
		{
			...
			if (!animClip.animation.IsEmpty())
			{
				...
				if (animID == -1)
				{
					...
					if (errorCallback)
					{
						sprintf_s(errorReport.error, MANN_ERROR_BUFFER_SIZE, "Animation not found: %s in layer: %d", animClip.animation.animRef.GetString(), l);

						(*errorCallback)(errorReport, errorCallbackContext);
					}
					CryLog("Animation not found: %s in layer: %d", animClip.animation.animRef.GetString(), l);
				}
			}
		}
	}
...
bool CAnimationDatabase::IsDuplicate(const CFragment* pFragmentA, const CFragment* pFragmentB) const
{
	...
	for (uint32 l = 0; l < numProcLayersA; l++)
	{
		...
		{
			const SProcClip &procClipA = procLayerA[c];
			const SProcClip &procClipB = procLayerB[c];
						...
						if(compareNumOptions == numOptions)
						{
								...
								&& ((pFragTagDef == NULL) || pFragTagDef->Contains(tags.fragmentTags, compareTags.fragmentTags)))
							{
								...
								if (allOptionsAreDuplicated)
								{
									...
									char tagList[512];
									char fragTagList[512];

									fragTagList[0] = 0;

									m_pTagDef->FlagsToTagList(compareTags.globalTags, tagList, 512);
									if(pFragTagDef)
									{
										pFragTagDef->FlagsToTagList(compareTags.fragmentTags, fragTagList, 512);
									}

									sprintf_s(mannErrorReport.error, MANN_ERROR_BUFFER_SIZE, "Fragment is duplicate of a parent fragment! Parent Tags - Global: '%s' Fragment: '%s'", tagList, fragTagList);
									mannErrorReport.tags = tags;
...
void CAnimationDatabase::EnumerateFragmentAnimAssets(const CFragment *pFragment, const IAnimationSet *animSet, SAnimAssetReport &assetReport, MannAssetCallback assetCallback, void *callbackContext) const
{
	...
	for (uint32 l=0; l<numLayers; l++)
	{
		...
		for (uint32 c=0; c<numClips; c++)
		{
			...
			if (!animClip.animation.IsEmpty())
			{
				assetReport.pAnimName = animClip.animation.animRef.GetString();
