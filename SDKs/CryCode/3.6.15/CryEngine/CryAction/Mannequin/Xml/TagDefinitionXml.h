...
struct STagDefinitionImportsInfo
{
	...
	STagDefinitionImportsInfo& FindOrAddImport( const char* const filename )
	{
		STagDefinitionImportsInfo* pImportInfo = FindImport( filename );
		if ( pImportInfo )
		{
			return *pImportInfo;
		}

		return AddImport( filename );
	}
	...
	STagDefinitionImportsInfo* FindImport( const char* const filename )
	{
		assert( filename );
		assert( filename[ 0 ] );

		const size_t importsCount = m_imports.size();
		for ( size_t i = 0; i < importsCount; ++i )
		{
			STagDefinitionImportsInfo& importsInfo = m_imports[ i ];
			const char* const importInfoFilename = importsInfo.GetFilename();
			const bool foundMatch = ( strcmp( filename, importInfoFilename ) == 0 );
			if ( foundMatch )
			{
				return &importsInfo;
			}
...
namespace mannequin
{
	bool LoadTagDefinition( const char* const normalizedFilename, CTagDefinition& tagDefinitionOut, bool isTags );
