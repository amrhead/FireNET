...
void CActionGame::UpdateImmersiveness()
{
	...
	// No secondary breaks in multiplayer and single player for c3 
# if (defined(PS3) || defined(XENON)) && defined(GAME_IS_CRYSIS3)
...
void CActionGame::OnCollisionLogged_MaterialFX( const EventPhys * pEvent )
{
	...
	if (!backface && impactVelSquared > (partImpThresh * partImpThresh))
	{
		...
		if (effectId != InvalidEffectId)
		{
				...
				if (pCollidedActor)
				{
...
#else
							//Filter sounds too, special events handled by the game
							params.playflags = MFX_PLAY_ALL &~ (MFX_PLAY_PARTICLES|MFX_PLAY_SOUND); 
#endif
						}
					}
					...
					else
					{
						if (!gEnv->bMultiplayer)
						{
							...
							if (pGameRules && pGameRules->IsClientFriendlyProjectile(params.src , params.trg))
							{
								params.playflags = MFX_PLAY_ALL &~ (MFX_PLAY_PARTICLES|MFX_PLAY_SOUND); 
							}
						}
			...
			params.soundDistanceMult = static_cast<float>(__fsel(mass0 - 1.0f, (finalparam * finalparam) + ((1.0f - finalparam) * .05f), 1.0f));
