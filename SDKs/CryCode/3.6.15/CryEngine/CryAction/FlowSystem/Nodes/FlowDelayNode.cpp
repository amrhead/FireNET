...
class CFlowFrameDelayNode : public CFlowBaseNode<eNCT_Instanced>
{
public:
	...
	void ProcessEvent( EFlowEvent event, SActivationInfo * pActInfo )
	{
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, 0))
			{
				...
				m_frameStamp = gEnv->pRenderer->GetFrameID();
			}
			break;
		case eFE_Update:
			if(m_frameStamp != gEnv->pRenderer->GetFrameID() - 1)
			{
				CryLogAlways("Frame delay node didn't skip 1 frame - activated frame %d, updated frame %d", m_frameStamp, gEnv->pRenderer->GetFrameID());
			}
			ActivateOutput( pActInfo, 0, m_InputData);
			pActInfo->pGraph->SetRegularlyUpdated( pActInfo->myID, false);
			break;
		}
	}
