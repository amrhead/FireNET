...
public:
	...
	{
		if ( !m_actInfo.pGraph || !m_actInfo.pGraph->IsEnabled() || m_actInfo.pGraph->IsSuspended() || !m_actInfo.pGraph->IsActive() )
			return;
		...
		case ENTITY_EVENT_XFORM:
			{
				OutputValues( uint32(event.nParam[0]) );
			}
			break;
		}
	}
	...
	void OutputValues( uint entityFlags )
	{
		ECoordSys coorSys = (ECoordSys)GetPortInt( &m_actInfo, IN_COORDSYS );
		...
		switch (coorSys)
		{
			case CS_WORLD:
			{
				if (bAny || entityFlags & ENTITY_XFORM_POS)
					ActivateOutput(&m_actInfo, OUT_POS, pEntity->GetWorldPos());

				if (bAny || entityFlags & ENTITY_XFORM_ROT)
				{
					ActivateOutput(&m_actInfo, OUT_ROTATE, Vec3(RAD2DEG(Ang3(pEntity->GetWorldAngles()))));
					const Matrix34& mat = pEntity->GetWorldTM();
					ActivateOutput( &m_actInfo, OUT_YDIR, mat.GetColumn1()); // forward -> mat.TransformVector(Vec3(0,1,0)) );
					ActivateOutput( &m_actInfo, OUT_XDIR, mat.GetColumn0()); // right   -> mat.TransformVector(Vec3(1,0,0)) );
					ActivateOutput( &m_actInfo, OUT_ZDIR, mat.GetColumn2()); // up      -> mat.TransformVector(Vec3(0,0,1)) );
				}

				if (bAny || entityFlags & ENTITY_XFORM_SCL)
					ActivateOutput(&m_actInfo, OUT_SCALE, pEntity->GetScale());
				break;
			}
			...
			{
				if (bAny || entityFlags & ENTITY_XFORM_POS)
					ActivateOutput(&m_actInfo, OUT_POS, pEntity->GetPos());

				if (bAny || entityFlags & ENTITY_XFORM_ROT)
				{
					ActivateOutput(&m_actInfo, OUT_ROTATE, Vec3(RAD2DEG(Ang3::GetAnglesXYZ(pEntity->GetLocalTM()))));
...
public:
	...
	virtual void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<EntityId>( "FarEntity",_HELP("Fartest entity") ),
...
//////////////////////////////////////////////////////////////////////////
class CFlowNode_EntityMaterialSerialize : public CFlowNode_EntityMaterial
{
public:
	CFlowNode_EntityMaterialSerialize( SActivationInfo *pActInfo ) : CFlowNode_EntityMaterial(  pActInfo )
	{
	}
...
//////////////////////////////////////////////////////////////////////////
class CFlowNode_EntityGetPlayer : public CFlowBaseNode<eNCT_Singleton>
{
public:
...
public:
	...
	bool ReadFloatValueFromMat( IMaterial* pSubMtl, IRenderShaderResources* pRendShaderRes, const char* paramName, float& floatValue )
	{
		...
		for (int i=0; i<shaderParams.size(); ++i)
		{
			...
			if (stricmp(param.m_Name, paramName) == 0)
			{
				...
				switch (param.m_Type)
				{
					case eType_BOOL: floatValue = param.m_Value.m_Bool; break;
					case eType_SHORT: floatValue = param.m_Value.m_Short; break;
					case eType_INT: floatValue = (float)param.m_Value.m_Int; break;
					case eType_FLOAT: floatValue = param.m_Value.m_Float; break;
				}
				return true;
			}
		}
	...
	//////////////////////////////////////////////////////////////////////////
	bool ObtainMaterialPtrs( int slot, int subMtlId, bool cloneMaterial, IMaterial*& pSubMtl, IRenderShaderResources*& pRenderShaderRes  )
	{
...
public:
	...
	virtual void ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo )
	{
		...
		{
			IMaterial* pOldMtl = (IMaterial*)pAttachObj->GetBaseMaterial();
			if (pOldMtl)
			{
		...
		if (bGet)
		{
			...
			for (int i=0; i< params.size(); ++i)
			{
				...
				if (stricmp(param.m_Name, paramNameFloat) == 0)
				{
					...
					case eType_INT: val = (float)param.m_Value.m_Int; break;
...
////////////////////////////////////////////////////////////////////////////////////////
class CFlowNodeEntityCloneMaterialOld : public CFlowBaseNode<eNCT_Singleton>
{
public:
...
// Flow node for setting entity render parameters (opacity, glow, motionblur, etc)
////////////////////////////////////////////////////////////////////////////////////////
class CFlowNodeMaterialShaderParam : public CFlowBaseNode<eNCT_Singleton>
{
public:
...
////////////////////////////////////////////////////////////////////////////////////////
class CFlowNodeMaterialShaderParamSerialize : public CFlowBaseNode<eNCT_Singleton>
{
public:
		...
		if (!validFloat)
		{
			for (int i=0; i< params.size(); ++i)
			{
				...
				if (stricmp(param.m_Name, paramNameFloat) == 0)
				{
					switch (param.m_Type)
					{
						case eType_BOOL: floatValue = param.m_Value.m_Bool; validFloat = true; break;
						case eType_SHORT: floatValue = param.m_Value.m_Short; validFloat = true; break;
						case eType_INT: floatValue = (float)param.m_Value.m_Int; validFloat = true; break;
						case eType_FLOAT: floatValue = param.m_Value.m_Float; validFloat = true; break;
						default: break;
					}
				}
			}
...
//////////////////////////////////////////////////////////////////////////
class CFlowNode_EntityMaterialLayer : public CFlowBaseNode<eNCT_Singleton>
{
public:
					...
					if (NULL != pArchetype)
					{
						...
						if(pEntity)
						{
							...
							if(pRenderProx)
							{
								...
								pArchetype->GetObjectVars()->getAttr("ViewDistRatio", nViewDistRatio);
...
REGISTER_FLOW_NODE( "Entity:AttachChild",CFlowNode_EntityAttachChild )
REGISTER_FLOW_NODE( "Entity:DetachThis",CFlowNode_EntityDetachThis )
