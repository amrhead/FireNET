...
const char* CAIProxy::GetVoiceLibraryName(const bool useForcedDefaultName /* = false */) const
{
	...
	{
		const string::size_type indexOfLastUnderscore = m_voiceLibrary.find_last_of('_');
		static const string::size_type npos = -1; // This is the value find_last_of will return if no '_' is found
		stack_string newVoiceLibraryName = m_voiceLibrary.substr(0, 
			indexOfLastUnderscore != npos ? indexOfLastUnderscore : m_voiceLibrary.size());
		return newVoiceLibraryName.Format("%s%s", newVoiceLibraryName.c_str(), "_test").c_str();
	}
	else
	{
...
int CAIProxy::Update(SOBJECTSTATE &state, bool bFullUpdate)
{
	...
	if (pMC)
	{
		...
		else
		{
			...
			{
				float speedMul = (float)__fsel(state.fMovementUrgency, 1.0f, -1.0f);
		...
		if (IWeapon *pSecWeapon=GetSecWeapon( state.requestedGrenadeType, &returnedGrenadeType ))
		{
			...
			if (pEnt)
			{
				...
				case eRGT_SMOKE_GRENADE:
					SendSignal(AISIGNAL_DEFAULT, "OnSmokeGrenadeThrown", pEnt, NULL); break;
				case eRGT_FLASHBANG_GRENADE:
					SendSignal(AISIGNAL_DEFAULT, "OnFlashbangGrenadeThrown", pEnt, NULL); break;
				case eRGT_FRAG_GRENADE:
					SendSignal(AISIGNAL_DEFAULT, "OnFragGrenadeThrown", pEnt, NULL); break;
				case eRGT_EMP_GRENADE:
					SendSignal(AISIGNAL_DEFAULT, "OnEMPGrenadeThrown", pEnt, NULL); break;
				}
