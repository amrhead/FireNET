...
template <typename F> struct __passinreg QuatTNS_tpl
{
	...
	ILINE QuatTNS_tpl() 
	{
		if (sizeof(F)==4)
		{
			uint32* p=alias_cast<uint32*>(&q.v.x); 		p[0]=F32NAN;p[1]=F32NAN;p[2]=F32NAN;p[3]=F32NAN; p[4]=F32NAN;p[5]=F32NAN;p[6]=F32NAN;p[7]=F32NAN;p[8]=F32NAN;p[9]=F32NAN;
		}
		if (sizeof(F)==8)
		{
			uint64* p=alias_cast<uint64*>(&q.v.x);		p[0]=F64NAN;p[1]=F64NAN;p[2]=F64NAN;p[3]=F64NAN; p[4]=F64NAN;p[5]=F64NAN;p[6]=F64NAN;p[7]=F64NAN;p[8]=F64NAN;p[9]=F64NAN;
		}
	}
#else
	ILINE QuatTNS_tpl()	{};
	...
	//CONSTRUCTOR: implement the copy/casting/assignment constructor:	
	template <typename F1> ILINE QuatTNS_tpl( const QuatTS_tpl<F1>& qts ) : 	q(qts.q), t(qts.t), s(qts.s)
	{	
	}

	ILINE QuatTNS_tpl& operator = (const QuatT_tpl<F> &qt) { 
		q=qt.q; t=qt.t; s=Vec3_tpl<F>(1);	return *this;
	}
	...
	ILINE void SetIdentity() 
	{
		q.SetIdentity(); t=Vec3_tpl<F>(ZERO); s=Vec3_tpl<F>(1);
	}
	...
	*/
	...
	{
		QuatTNS_tpl<F> d;  d.SetNLerp(p,q,t); 	return d;
	}
