...
DEFINE_ENUM( ETrinary,
	Both,
	If_True,
	If_False
)

inline bool operator *(ETrinary t1, ETrinary t2)
{
	return (t1 | t2) != 3;
}

inline bool operator *(ETrinary t, bool b)
{
	return t != (ETrinary::If_True + (int)b);
}
	...
	: ZeroInit<ParticleParams>
{
	...
	// <Group=Appearance>
	#define _EBlend(_) \
		_(AlphaBased,			 = OS_ALPHA_BLEND) \
		_(Additive,        = OS_ADD_BLEND) \
		_(Multiplicative,  = OS_MULTIPLY_BLEND) \
		_(Opaque,          = 0) \
		_(_ColorBased,     = OS_ADD_BLEND) \
		_(_None,           = 0) \
	...
	struct STextureTiling
	{
		...
		float GetAnimPos(float fAge, float fRelativeAge) const
		{
			...
			{
				float fAnimPos = fAge * fAnimFramerate / nAnimFramesCount;
				if (eAnimCycle().Loop)
					return fmod(fAnimPos, 1.f);
				else if (eAnimCycle().Mirror)
