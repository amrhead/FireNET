...
const CTypeInfo::CVarInfo* CStructInfo::NextSubVar(const CVarInfo* pPrev, bool bRecurseBase) const
{
	...
	if (bRecurseBase)
	{
		...
		for (; pBase < Vars.end() && pBase->IsInline(); ++pBase)
		{
			if (pBase->Type.HasSubVars())
			{
				if (!pPrev)
					return pBase->Type.NextSubVar(pPrev, true);
				assert(pPrev->Offset >= pBase->Offset);
				if (pPrev->Offset + pPrev->GetSize() < pBase->Offset + pBase->GetSize())
					return pBase->Type.NextSubVar(pPrev, true);
				pPrev = pBase;
			}
		}
	...
	assert(pPrev >= Vars.begin() && pPrev < Vars.end());
	assert(pPrev->Offset >= 0 && pPrev->Offset + pPrev->GetSize() <= Size);
...
//////////////////////////////////////////////////////////////////////
// TEnumInfo<> implementation

template<class TEnumDef>
bool TEnumInfo<TEnumDef>::ToValue(const void* data, void* value, const CTypeInfo& typeVal) const
{
	return ConvertInt(value, typeVal, ReadInt(data));
}

template<class TEnumDef>
bool TEnumInfo<TEnumDef>::FromValue(void* data, const void* value, const CTypeInfo& typeVal) const
{
	TValue val;
	if (ConvertInt(val, value, typeVal))
	{
		if (def().ToName(val))
			return WriteInt(data, val);
	}
	return false;
}

template<class TEnumDef>
string TEnumInfo<TEnumDef>::ToString(const void* data, FToString flags, const void* def_data) const
{
	TValue val = ReadInt(data);

	if (flags.SkipDefault)
	{
		TValue def_val = def_data ? ReadInt(def_data) : 0;
		if (val == def_val)
			return string();
	}

	if (cstr sName = def().ToName(val))
		return sName;
...
template<class TEnumDef>
bool TEnumInfo<TEnumDef>::FromString(void* data, cstr str, FFromString flags) const
{
	if (!*str)
	{
		if (flags.SkipEmpty)
			return true;
		return WriteInt(data, 0);
	}

	for (int i = 0, count = def().Count(); i < count; i++)
	{
		if (def().MatchName(i, str))
			return WriteInt(data, def().Value(i));
	}
	...
	bool b;
	if (::FromString(b, str))
		return WriteInt(data, b);

	return false;
}

//////////////////////////////////////////////////////////////////////
// CSimpleEnumInfo implementation

CSimpleEnumInfo::CSimpleEnumInfo( cstr name, size_t size, size_t align, Array<cstr> names, char* s )
	: TEnumInfo(name, size, align),
		asNames(names)
{
...
CEnumInfo::CEnumInfo( cstr name, size_t size, size_t align, Array<CEnumElem> elems )
	: TEnumInfo(name, size, align), 
		Elems(elems),
		MinValue(0), bRegular(true), nPrefixLength(0)
{
...
bool CEnumInfo::MatchName(uint i, cstr str) const
{
...
cstr CEnumInfo::ToName(TValue value) const
{
