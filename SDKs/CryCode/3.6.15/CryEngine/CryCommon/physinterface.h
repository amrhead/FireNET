...
public:
	...
	ILINE CMemStream(void *pbuf, int sz, bool swap) {
		m_pBuf=(char*)pbuf; m_nSize=sz; m_iPos=0; bDeleteBuf=false; bSwapEndian=swap;	bMeasureOnly=0;
	}
	ILINE CMemStream() {
...
struct pe_params_pos : pe_params { //	Sets position and orientation of entity
	enum entype { type_id=ePE_params_pos };
	pe_params_pos() {
		type=type_id; MARK_UNUSED pos,scale,q,iSimClass; pMtx3x4=0;pMtx3x3=0; bRecalcBounds=1; bEntGridUseOBB=0;
	}
...
struct pe_params_sensors : pe_params { // Attaches optional ray sensors to an entity; only living entities support it
...
struct pe_player_dimensions : pe_params {
	...
	pe_player_dimensions() : dirUnproj(0,0,1),maxUnproj(0) { 
		type=type_id; MARK_UNUSED sizeCollider,heightPivot,heightCollider,heightEye,heightHead,headRadius,bUseCapsule,groundContactEps; 
	}
...
struct pe_player_dynamics : pe_params {
	...
	pe_player_dynamics() {
		type=type_id; MARK_UNUSED kInertia,kInertiaAccel,kAirControl,gravity,gravity.z,nodSpeed,mass, bSwimming,surface_idx,bActive,collTypes,pLivingEntToIgnore;
...
struct pe_status_nparts : pe_status {	// GetStuts will return the number of parts
...
struct SVoxGridParams : SMeshBVParams { // voxel grid is a regular 3d grid collision test acceleration structure
	Vec3 origin;
	Vec3 step;
	Vec3i size;
};
...
struct EventPhysMono : EventPhys { // base for one-entity events, ids 0-2
	IPhysicalEntity *pEntity;
	void *pForeignData;
	int iForeignData;
};

struct EventPhysStereo : EventPhys { // base for two-entity events, ids 3-14
	IPhysicalEntity *pEntity[2];
	void *pForeignData[2];
	int iForeignData[2];
};

struct EventPhysBBoxOverlap : EventPhysStereo {	// generated by triggers and parts with geom_log_interactions
...
struct EventPhysPostStep : EventPhysMono { // entity has just completed its step
...
struct EventPhysCreateEntityPart : EventPhysMono { // a part broke off an existing entity
...
struct EventPhysArea : EventPhysMono { // for callback phys areas
...
UNIQUE_IFACE struct IPhysicalWorld {
	...
	struct SRWIParams {
		...
		{
			memset(this,0,sizeof(*this)); objtypes=ent_all; flags=rwi_stop_at_pierceable;
			org=_org; dir=_dir; objtypes=_objtypes; flags=_flags; collclass=_collclass; hits=_hits; nMaxHits=_nMaxHits;
			pForeignData=_pForeignData; iForeignData=_iForeignData; phitLast=_phitLast; pSkipEnts=_pSkipEnts; nSkipEnts=_nSkipEnts;
			return *this;
		}
	...
	struct SPWIParams {
		...
		const SPWIParams& Init (int _itype, const primitives::primitive *_pprim, const Vec3 &_sweepDir=Vec3(ZERO), int _entTypes=ent_all, 
			geom_contact **_ppcontact=0, int _geomFlagsAll=0,int _geomFlagsAny=geom_colltype0|geom_colltype_player, const SCollisionClass& _collclass=SCollisionClass(0,0),
			intersection_params *_pip=0, void *_pForeignData=0, int _iForeignData=0, IPhysicalEntity **_pSkipEnts=0,int _nSkipEnts=0)
		{
			memset(this,0,sizeof(*this)); 
			entTypes=ent_all;	geomFlagsAny=geom_colltype0|geom_colltype_player;
			lockContacts.prw = &lockContacts.iActive;
			itype=_itype; pprim=_pprim; sweepDir=_sweepDir; entTypes=_entTypes;
			ppcontact=_ppcontact; geomFlagsAll=_geomFlagsAll; geomFlagsAny=_geomFlagsAny; collclass=_collclass; pip=_pip;
			pForeignData=_pForeignData; iForeignData=_iForeignData; pSkipEnts=_pSkipEnts; nSkipEnts=_nSkipEnts;
			return *this;
		}
