...
UNIQUE_IFACE struct IGameStartup
{
	...
	// Description:
	//		Returns an array of (case-insensitive) names of pak files loaded by the game in release mode, or NULL. The array must be terminated by a NULL-pointer.
	//		All specified file names are relative to the game folder
	//		Note: If NULL is returned, the default list of PAK files is used instead
	virtual const char** GetPakFiles() const { return NULL; }
