...
struct SAIEVENT
{
	...
	SAIEVENT() : bFuzzySight(false), bPathFound(false), bSetObserver(false), nDeltaHealth(0), fThreat(0.f),
...
struct IAICommunicationHandler
{
	...
	//DEPREC: [IAgent.h]
	//virtual tSoundID PlaySound(CommPlayID playID, const char* name, IEventListener* listener = 0) = 0;
	//virtual void StopSound(tSoundID soundID) = 0;

	//virtual tSoundID PlayVoice(CommPlayID playID, const char* name, ELipSyncMethod lipSyncMethod, IEventListener* listener = 0) = 0;
