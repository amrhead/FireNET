...
namespace spline
{
	...
	template <> inline void Zero( float &val ) { val = 0.0f; }
	template <> inline void Zero( Vec2 &val ) { val = Vec2(0.0f,0.0f); }
	template <> inline void Zero( Vec3 &val ) { val = Vec3(0.0f,0.0f,0.0f); }
	template <> inline void Zero( Quat &val ) { val.SetIdentity(); }
	...
	****************************************************************************/
	...
	public:
		...
		void comp_deriv()
		{
			...
			// Initialise all endpoint slopes to 0.
			this->key(0).ds = this->key(0).dd = this->key(last).ds = this->key(last).dd = value_type(0.f);
			...
			if (bCLAMP)
			{
				...
				{
					this->ds(i) = this->dd(i) = minmag(this->value(i) - this->value(i-1), this->value(i+1) - this->value(i));
				}
				...
				for (int i = 0; i <= last; ++i)
				{
					...
					if (i < last && this->GetOutTangentType(i) == SPLINE_KEY_TANGENT_LINEAR)
					{
						...
						if (this->GetInTangentType(i+1) != SPLINE_KEY_TANGENT_LINEAR)
					...
					if (i > 0 && this->GetInTangentType(i) == SPLINE_KEY_TANGENT_LINEAR)
					{
						...
						if (this->GetOutTangentType(i-1) != SPLINE_KEY_TANGENT_LINEAR)
	...
	class	CatmullRomSpline : public TSplineSlopes< T,Key,bRangeLimit >
	{
	protected:
	...
	//////////////////////////////////////////////////////////////////////////
	template <class T,class Key = SplineKey<T> >
	class	BezierSpline : public TSpline< Key,BezierBasis >
	{
	public:
	...
	template <typename spline_type> struct SSplineBackup : public ISplineBackup, public spline_type
	{
		int refCount;

		SSplineBackup(spline_type const& s): spline_type(s), refCount(0) {}
