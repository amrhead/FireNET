...
template <typename F> struct __passinreg Matrix34_tpl
{
	...
	template<class F1> ILINE Matrix34_tpl<F>(	F1 v00, F1 v01, F1 v02, F1 v03,	F1 v10, F1 v11, F1 v12, F1 v13,	F1 v20, F1 v21, F1 v22, F1 v23)
	{ 		
		m00=F(v00); m01=F(v01); m02=F(v02); m03=F(v03); 
		m10=F(v10); m11=F(v11); m12=F(v12); m13=F(v13); 
		m20=F(v20); m21=F(v21); m22=F(v22); m23=F(v23); 
	}
	...
	ILINE Matrix34_tpl<F>( const Matrix34_tpl<F>& m )
  {
	...
	//Matrix34 m=m34d;  
	template<class F1> ILINE Matrix34_tpl<F>( const Matrix34_tpl<F1>& m )
  {
		assert(m.IsValid());
	...
	//Needs to be 'explicit' because we loose the translation vector in the conversion process 
	//Matrix34(m44r);
	template<class F1> ILINE explicit Matrix34_tpl<F>(const Matrix44_tpl<F1>& m )
	{
		assert(m.IsValid());
		m00=F(m.m00);		m01=F(m.m01);		m02=F(m.m02);		m03=F(m.m03);	
		m10=F(m.m10);		m11=F(m.m11);		m12=F(m.m12);		m13=F(m.m13);
	...
	//Needs to be 'explicit' because we loose float-precision in the conversion process 
	//Matrix34(QuatTd);
	template<class F1> explicit ILINE Matrix34_tpl<F>( const Quat_tpl<F1>& q ) 
	{ 
		*this=Matrix33_tpl<F>(q); 
	}
	...
	explicit ILINE Matrix34_tpl<F>( const QuatTS_tpl<F> &q ) 
	{
		...
		F xx=1-v2.x*q.q.v.x;	F yy=v2.y*q.q.v.y;	F xw=v2.x*q.q.w;
		F xy=v2.y*q.q.v.x;		F yz=v2.z*q.q.v.y;	F yw=v2.y*q.q.w;
		F xz=v2.z*q.q.v.x;		F zz=v2.z*q.q.v.z;	F zw=v2.z*q.q.w;
		m00=(1-yy-zz)*q.s;		m01=(xy-zw)*q.s;		m02=(xz+yw)*q.s;	m03=q.t.x;
		m10=(xy+zw)*q.s;			m11=(xx-zz)*q.s;		m12=(yz-xw)*q.s;	m13=q.t.y;
		m20=(xz-yw)*q.s;			m21=(yz+xw)*q.s;		m22=(xx-yy)*q.s;	m23=q.t.z;
	}
	//CONSTRUCTOR for different float-types. It converts a QuatTS into a Matrix34. 
	//Needs to be 'explicit' because we loose float-precision in the conversion process 
	//Matrix34(QuatT);
	template<class F1> ILINE explicit Matrix34_tpl<F>( const QuatTS_tpl<F1> &q ) 
	{
		assert(q.q.IsValid());
	...
	explicit ILINE Matrix34_tpl<F>( const QuatTNS_tpl<F> &q ) 
	{
		...
		F xy=v2.y*q.q.v.x;		F yz=v2.z*q.q.v.y;	F yw=v2.y*q.q.w;
		F xz=v2.z*q.q.v.x;		F zz=v2.z*q.q.v.z;	F zw=v2.z*q.q.w;
		m00=(1-yy-zz)*q.s.x;		m01=(xy-zw)*q.s.y;		m02=(xz+yw)*q.s.z;	m03=q.t.x;
		m10=(xy+zw)*q.s.x;			m11=(xx-zz)*q.s.y;		m12=(yz-xw)*q.s.z;	m13=q.t.y;
		m20=(xz-yw)*q.s.x;			m21=(yz+xw)*q.s.y;		m22=(xx-yy)*q.s.z;	m23=q.t.z;
	}
	//CONSTRUCTOR for different float-types. It converts a QuatTNS into a Matrix34. 
	//Needs to be 'explicit' because we loose float-precision in the conversion process 
	//Matrix34(QuatT);
	template<class F1> ILINE explicit Matrix34_tpl<F>( const QuatTNS_tpl<F1> &q ) 
	{
		assert(q.q.IsValid());
		Vec3_tpl<F1> v2=q.q.v+q.q.v;
		F1 xx=1-v2.x*q.q.v.x;		F1 yy=v2.y*q.q.v.y;		F1 xw=v2.x*q.q.w;
		F1 xy=v2.y*q.q.v.x;			F1 yz=v2.z*q.q.v.y;		F1 yw=v2.y*q.q.w;
		F1 xz=v2.z*q.q.v.x;			F1 zz=v2.z*q.q.v.z;		F1 zw=v2.z*q.q.w;
		m00=F((1-yy-zz)*q.s.x);		m01=F((xy-zw)*q.s.y);		m02=F((xz+yw)*q.s.z); m03=F(q.t.x);
		m10=F((xy+zw)*q.s.x);			m11=F((xx-zz)*q.s.y);		m12=F((yz-xw)*q.s.z);	m13=F(q.t.y);
		m20=F((xz-yw)*q.s.x);			m21=F((yz+xw)*q.s.y);		m22=F((xx-yy)*q.s.z);	m23=F(q.t.z);
	}
	...
	ILINE explicit Matrix34_tpl<F>( const DualQuat_tpl<F> &q ) 
	{
		...
		F xx=1-v2.x*q.nq.v.x;	F yy=v2.y*q.nq.v.y;	F xw=v2.x*q.nq.w;
		F xy=v2.y*q.nq.v.x;		F yz=v2.z*q.nq.v.y;	F yw=v2.y*q.nq.w;
		F xz=v2.z*q.nq.v.x;		F zz=v2.z*q.nq.v.z;	F zw=v2.z*q.nq.w;
	...
	//Needs to be 'explicit' because we loose float-precision in the conversion process 
	//Matrix34(QuatT);
	template<class F1> ILINE explicit Matrix34_tpl<F>( const DualQuat_tpl<F1> &q ) 
	{
		assert((fabs_tpl(1-(q.nq|q.nq)))<0.01); //check if unit-quaternion
	...
	void SetIdentity( void )
  {
	...
	*/
	...
	{	
		Matrix34_tpl<F> m34; m34.SetTranslationMat(v); return m34; 	
	}
	...
	*/
	...
	{
		Matrix34_tpl<F> m34;	m34.Set(s,q,t); return m34;	
	}
	...
	*/
	...
	{ 	
		Matrix34_tpl<F> m34;  m34.SetScale(s,t);	return m34;	
	}
